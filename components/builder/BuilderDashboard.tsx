...
export default function BuilderDashboard() {
  ...
  // step from query (?step=1..4) — Pages Router safe
  ...
  const setStep = (next: string | null) => {
    ...
    if (next) (q as any).step = next;
    else delete (q as any).step;
    ...
    <div className="min-h-screen w-full font-movatif" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      <main className="flex-1 w-full px-4 sm:px-6 pt-6 pb-24">
        ...
        <section className="relative p-5 md:p-6 mb-6" style={PANEL}>
          ...
          {/* Search */}
          <div className="mt-4 flex items-center gap-2 rounded-[14px] px-4 py-3 text-[15px] max-w-xl" style={CARD}>
              ...
              try {
                ...
                if (Array.isArray(cloud)) {
                  const idx = cloud.findIndex(
                    (x) => (x.assistantId || x.id) === (selectedBot.assistantId || selectedBot.id)
                  );
                  if (idx >= 0) {
  ...
  return (
    <button
      ...
      style={{
        ...CARD,
        borderStyle: 'dashed',
        background: 'linear-gradient(180deg, color-mix(in oklab, var(--brand) 8%, transparent), transparent)',
      }}
    >
      <div className="p-6">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ border: '2px dashed var(--brand-weak)', color: 'var(--brand)', background: 'transparent' }}
          >
        ...
        <div className="mt-5">
          <span className="inline-flex items-center gap-2 rounded-[12px] px-3 py-2 text-sm transition" style={BTN_PANEL}>
    ...
    <div className="w-full overflow-hidden" style={CARD}>
      <div className="flex items-start justify-between p-5 pb-0">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-11 h-11 rounded-[12px] grid place-items-center" style={{ border: '2px solid var(--brand-weak)' }}>
        ...
        <button onClick={onCustomize} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-[10px] text-xs transition" style={BTN_PANEL} title="Customize look">
          <SlidersHorizontal className="w-3.5 h-3.5" />
          Style
        </button>
      </div>

      <div className="relative mt-4 h-[160px] md:h-[180px]" style={{ borderTop: '1px dashed var(--brand-weak)', borderBottom: '1px dashed var(--brand-weak)' }}>
        {/* @ts-ignore */}
        <Bot3D
          className="h-full"
        ...
        <div className="absolute right-3 top-3">
          <button className="p-2 rounded-[10px]" style={BTN_PANEL} title="More" onClick={onOpen}>
      ...
      <div className="p-5">
        <div className="grid gap-2 md:grid-cols-3">
          ...
          <RowStat label="Details" value={[bot.industry, bot.language].filter(Boolean).join(' · ') || '—'} />
        </div>

        <div className="mt-5 flex items-center gap-3">
          <button onClick={onOpen} className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[12px] text-sm transition" style={BTN_PANEL}>
            Open <ArrowRight className="w-4 h-4" />
          </button>
          <button
            onClick={onDelete}
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[12px] text-sm transition"
            style={{ background: 'rgba(255,120,120,.12)', border: '1px solid rgba(255,120,120,.35)', color: 'rgba(255,170,170,.95)', boxShadow: 'var(--shadow-soft)' }}
          >
      ...
      <div className="relative w-full max-w-[1280px] max-h-[88vh] flex flex-col" style={FRAME_STYLE}>
        <div className="flex items-center justify-between px-6 py-4 rounded-t-[30px]" style={HEADER_BORDER}>
          ...
          <div className="flex items-center gap-2">
            <button onClick={copyAll} className="inline-flex items-center gap-2 rounded-[14px] px-3 py-2 text-xs border" style={{ background: 'var(--card)', borderColor: 'var(--border)', color: 'var(--text)' }}>
              <Copy className="w-3.5 h-3.5" /> Copy
            </button>
            <button onClick={downloadTxt} className="inline-flex items-center gap-2 rounded-[14px] px-3 py-2 text-xs border" style={{ background: 'var(--card)', borderColor: 'var(--border)', color: 'var(--text)' }}>
        ...
        <div className="px-6 py-4 rounded-b-[30px]" style={{ borderTop: '1px solid var(--border)', background: 'var(--card)' }}>
          <div className="flex justify-end">
            <button onClick={onClose} className="px-5 py-2 rounded-[14px] font-semibold" style={{ background: 'var(--brand)', color: '#fff', boxShadow: '0 0 18px rgba(0,255,194,.20)' }}>
              Close
            </button>
          </div>
        </div>
...
function ConfirmDelete({
  bot, onCancel, onConfirm,
}: { bot: Bot; onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] bg-black/70 grid place-items-center px-4">
      <div className="w-full max-w-md rounded-[22px] p-6" style={PANEL}>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-lg font-semibold" style={{ color: 'var(--text)' }}>Delete this build?</div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onCancel} className="px-5 py-2 rounded-[14px]" style={CARD}>Cancel</button>
          <button
            onClick={onConfirm}
  ...
  return (
    <div className="rounded-[16px] p-4" style={PANEL}>
      ...
      <div className="grid lg:grid-cols-3 gap-4 text-sm">
        ...
        <div>
          <div className="font-medium mb-1">account API: <code>/api/chatbots/list</code></div>
          <pre className="max-h-[300px] overflow-auto rounded-[10px] p-2" style={CARD}>
      ...
      <div className="mt-3 text-xs" style={{ color: 'var(--text-muted)' }}>
        Anything saved via Step 4 → <code>/api/chatbots/save</code> will appear here and sync to other devices signed into the same account.
      </div>
    </div>
  );
}
