...
function splitStep3IntoSections(
  step3Raw?: string
): SplitSection[] | null {
  ...
  while ((m = HEADING_REGEX.exec(step3Raw)) !== null) {
    ...
    const label =
      rawLabel === 'AI  DESCRIPTION'
        ? ('AI DESCRIPTION' as PromptSectionKey)
        : rawLabel;
  ...
  for (let i = 0; i < matches.length; i++) {
    const h = matches[i];
    const nextStart =
      i + 1 < matches.length ? matches[i + 1].start : step3Raw.length;
    out.push({
      key: h.label,
...
export default function BuilderDashboard() {
  ...
  const mode =
    search.get('mode') === 'signin' ? 'signin' : 'signup';
  ...
  const step =
    rawStep && ['1', '2', '3', '4'].includes(rawStep)
      ? rawStep
      : null;

  const [query, setQuery] = useState('');
  const [bots, setBots] = useState<Bot[]>([]);
  const [customizingId, setCustomizingId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem('builder:cleanup') === '1') {
        ['builder:step1', 'builder:step2', 'builder:step3'].forEach(
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {step === '1' && (
              <Step1AIType
                onNext={() =>
                  doPhase('Preparing model settings…', () =>
                    setStep('2')
                  )
                }
              />
            ...
            {step === '2' && (
              <Step2ModelSettings
                onBack={() =>
                  doPhase('Going back…', () => setStep('1'))
                }
                onNext={() =>
                  doPhase('Opening prompt editor…', () =>
                    setStep('3')
                  )
                }
              />
            ...
            {step === '3' && (
              <Step3PromptEditor
                onBack={() =>
                  doPhase('Returning to settings…', () =>
                    setStep('2')
                  )
                }
                onNext={() =>
                  doPhase('Generating overview…', () =>
                    setStep('4')
                  )
                }
              />
        ...
        >
            ...
            {filtered.map((bot, i) => (
              ...
              >
                <BuildCard
                  bot={bot}
                  accent={
                    bot.appearance?.accent || accentFor(bot.id)
                  }
                  onOpen={() =>
                    doPhase('Opening prompt…', () =>
                      setViewId(bot.id)
                    )
                  }
                  onDelete={() =>
          ...
          {filtered.length === 0 && (
            <div className="mt-12 text-center text-white/60">
              ...
              <span className="text-[#00ffc2]">Create a Build</span>{' '}
              to get started.
            </div>
          )}
        </motion.div>
  ...
  return (
    ...
    >
      ...
      >
        <Plus
          className="w-10 h-10"
          style={{ color: '#6af7d1', opacity: 0.9 }}
        />
      </div>
...
const accentFor = (id: string) =>
  palette[
    Math.abs(
      [...id].reduce((h, c) => h + c.charCodeAt(0), 0)
    ) % palette.length
  ];
      ...
      <div className="h-48 border-b border-white/10 overflow-hidden relative">
        ...
        >
          ...
          <Bot3D
            ...
            antenna={
              ap.hasOwnProperty('antenna')
                ? Boolean((ap as any).antenna)
                : true
            }
            withBody={
              ap.hasOwnProperty('withBody')
                ? Boolean(ap.withBody)
                : true
            }
            idle={
              ap.hasOwnProperty('idle')
                ? Boolean(ap.idle)
                : true
            }
          />
