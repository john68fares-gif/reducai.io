      ...
      const normalize = (k: string) => {
        ...
        if (v && typeof v === 'object') {
          (['name', 'industry', 'language'] as const).forEach((key) => {
            if (v[key] !== undefined)
              v[key] = typeof v[key] === 'string' ? v[key] : '';
          });
    ...
    <div className="min-h-screen w-full text-white font-movatif bg-[#0b0c10]">
      <main className="flex-1 w-full px-4 sm:px-6 pt-10 pb-24">
        <div className="flex items-center justify-between mb-7">
          ...
          <button
            onClick={() => router.replace('/builder?step=1')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-7">
          <CreateCard onClick={() => router.replace('/builder?step=1')} />
      ...
      {viewedBot && (
        // You referenced PromptOverlay in earlier versions; if you still need it,
        // keep the component and this line will work.
        // <PromptOverlay bot={viewedBot} onClose={() => setViewId(null)} />
        <div /> // placeholder if PromptOverlay isn't included in this file
      )}
    </div>
  );
}
