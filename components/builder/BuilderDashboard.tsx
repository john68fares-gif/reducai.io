...
const STORAGE\_KEYS = \['chatbots', 'agents', 'builds'];
const SAVE\_KEY = 'chatbots';
...
try { localStorage.setItem(SAVE\_KEY, JSON.stringify(bots)); } catch {}
}
...
const DISPLAY\_TITLES: Record\<PromptSectionKey, string> = {
...
HEADING\_REGEX.lastIndex = 0;
while ((m = HEADING\_REGEX.exec(step3Raw)) !== null) {
const rawLabel = (m\[1] || '')
.toUpperCase()
.replace(/\s\*&\s\*/g, ' AND ')
.replace(/\s+/g, ' ') as PromptSectionKey;
const label = rawLabel === 'AI  DESCRIPTION' ? ('AI DESCRIPTION' as PromptSectionKey) : rawLabel;
matches.push({ start: m.index, end: HEADING\_REGEX.lastIndex, label });
}
...
out.push({
key: h.label,
title: DISPLAY\_TITLES\[h.label] || h.label,
...
if (STORAGE\_KEYS.includes(e.key || '')) setBots(loadBots());
};
...
if (step) {
return ( <div className="min-h-screen w-full text-white font-movatif bg-[#0b0c10]"> <main className="w-full min-h-screen">
...
{step === '4' && \<Step4Overview onBack={() => setStep('3')} onFinish={() => setStep(null)} />} </main> </div>
);
}

return ( <div className="min-h-screen w-full text-white font-movatif bg-[#0b0c10]"> <main className="flex-1 w-full px-4 sm:px-6 pt-10 pb-24"> <div className="flex items-center justify-between mb-7"> <h1 className="text-2xl md:text-3xl font-semibold">Builds</h1>
\<button
onClick={() => router.push('/builder?step=1')}
className="px-4 py-2 rounded-\[10px] bg-\[#00ffc2] text-black font-semibold shadow-\[0\_0\_10px\_rgba(106,247,209,0.28)] hover\:brightness-110 transition"
\>
Create a Build </button> </div>
...
const FRAME\_STYLE: React.CSSProperties = {
background: 'rgba(13,15,17,0.95)',
border: '2px dashed rgba(106,247,209,0.3)',
boxShadow: '0 0 40px rgba(0,0,0,0.7)',
borderRadius: 30,
};
const HEADER\_BORDER = { borderBottom: '1px solid rgba(255,255,255,0.4)' };
const CARD\_STYLE: React.CSSProperties = {
background: '#101314',
...
return (
\<div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.5)' }}> <div className="relative w-full max-w-[1280px] max-h-[88vh] flex flex-col" style={FRAME_STYLE}> <div className="flex items-center justify-between px-6 py-4 rounded-t-[30px]" style={HEADER_BORDER}> <div className="min-w-0"> <h2 className="text-white text-xl font-semibold truncate">Prompt</h2> <div className="text-white/90 text-xs md:text-sm truncate">
{\[bot.name, bot.industry, bot.language].filter(Boolean).join(' · ') || '—'} </div> </div>
...
\>
\<Plus className="w-10 h-10" style={{ color: '#6af7d1', opacity: 0.9 }} /> </div> <div className="text-[20px]">Create a Build</div> <div className="text-[13px] text-white/65 mt-2">Start building your AI assistant</div> </button>
);
}
