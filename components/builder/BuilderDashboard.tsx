...
const Bot3D = dynamic(() => import('./Bot3D.client'), {
  ssr: false,
  loading: () => <div className="h-full w-full" style={{ background: 'var(--panel)' }} />,
});
...
type Bot = {
  id: string; name: string; industry?: string; language?: string; model?: string; description?: string; prompt?: string;
...
function splitStep3IntoSections(step3Raw?: string): SplitSection[] | null {
  ...
  while ((m = HEADING_REGEX.exec(step3Raw)) !== null) {
    const rawLabel = (m[1] || '').toUpperCase().replace(/\s*&\s*/g, ' AND ').replace(/\s+/g, ' ') as PromptSectionKey;
...
export default function BuilderDashboard() {
  ...
  if (step) {
    return (
      <div className="min-h-screen w-full" style={{ color: 'var(--text)', background: 'var(--bg)' }}>
    ...
    <div className="min-h-screen w-full" style={{ color: 'var(--text)', background: 'var(--bg)' }}>
      <main className="flex-1 w-full px-4 sm:px-6 pt-10 pb-24">
        <div className="flex items-center justify-between mb-7">
          ...
          <button onClick={() => router.push('/builder?step=1')}
                  className="btn-brand rounded-[10px] px-4 py-2 font-semibold shadow-[0_0_10px_var(--ring)] hover:brightness-110 transition">
            Create a Build
          </button>
        </div>

        <div className="mb-8">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search projects and builds…"
                 className="w-full rounded-[10px] input px-5 py-4 text-[15px] outline-none" />
        </div>
      ...
      {selectedBot && (
        <CustomizeModal
          ...
          onSaveDraft={(name, ap) => {
            // drafts can remain local & namespaced if you want; optional to implement later
          }}
        />
      )}
      ...
      <div className="relative w-full max-w-[1280px] max-h-[88vh] flex flex-col" style={FRAME_STYLE}>
        ...
        <div className="flex items-center justify-between px-6 py-4 rounded-t-[30px]" style={HEADER_BORDER}>
          <div className="min-w-0">
            <h2 className="text-xl font-semibold truncate">Prompt</h2>
          ...
          <div className="flex items-center gap-2">
            <button onClick={copyAll} className="btn-ghost px-3 py-2 text-xs" style={{ background: 'var(--card)' }} title="Copy">
              <Copy className="w-3.5 h-3.5" /> Copy
            </button>
            <button onClick={downloadTxt} className="btn-ghost px-3 py-2 text-xs" style={{ background: 'var(--card)' }} title="Download">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {!bot.prompt ? (
            <div className="p-5" style={{ ...CARD_STYLE, color: 'var(--text-muted)' }}>
              ...
              {sections.map((sec, i) => (
                <div key={i} style={CARD_STYLE} className="overflow-hidden">
                  ...
                  <div className="px-5 pb-5">
                    <pre className="whitespace-pre-wrap text-sm leading-6">{sec.text}</pre>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={CARD_STYLE} className="p-5">
              <pre className="whitespace-pre-wrap text-sm leading-6">{bot.prompt}</pre>
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 rounded-b-[30px]" style={{ borderTop: '1px solid var(--border)', background: 'var(--card)' }}>
          <div className="flex justify-end">
            <button onClick={onClose} className="btn-brand px-5 py-2 rounded-[14px] font-semibold">Close</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute -top-[28%] -left-[28%] w-[70%] h-[70%] rounded-full"
           style={{ background: 'radial-gradient(circle, var(--brand-weak) 0%, transparent 70%)', filter: 'blur(38px)' }} />
      ...
      <div className="text-[13px]" style={{ color: 'var(--text-muted)', marginTop: 8 }}>Start building your AI assistant</div>
    </button>
  );
}
    ...
    >
      ...
      <div className="h-48 border-b" style={{ borderColor: 'var(--border)' }}>
        <button
          ...
          style={{ background: 'var(--panel)', border: '1px solid var(--brand-weak)', boxShadow: 'var(--shadow-soft)' }}
        >
          <SlidersHorizontal className="w-3.5 h-3.5" />
          Customize
        </button>

        <ErrorBoundary fallback={<div className="h-full w-full" style={{ background: 'var(--panel)' }} />}>
          {/* @ts-ignore */}
          <Bot3D
            className="h-full"
            ...
            idle={ap.hasOwnProperty('idle') ? Boolean(ap.idle) : true}
          />
        </ErrorBoundary>
      </div>
