...
const CARD: React.CSSProperties = {
  ...
  background: 'rgba(13,15,17,.96)', border: '1px solid rgba(106,247,209,.28)',
...
}: { open: boolean; mode: 'signup'|'signin'; userId: string; onDone: () => void }) {
          ...
          <div className="min-h-screen grid place-items-center p-6">
            <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:12}} transition={{duration:.28}}
              ...
              <div className="flex items-center justify-between">
                ...
                {mode === 'signup' && <div className="text-sm text-white/70">{pct}%</div>}
              </div>
              {mode === 'signup' && (
                  ...
                  {step === 1 && (
                    <>
                      ...
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {OPTIONS.map(op => (
                          <button key={op} onClick={()=>setHeardFrom(op)}
