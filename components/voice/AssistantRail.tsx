...
export default function AssistantRail({
  ...
  // tell the page to recompute margins whenever our width/collapse changes
  useEffect(() => {
    const ping = () => window.dispatchEvent(new Event('voice:layout:ping'));
    ping();
    const ro = new ResizeObserver(ping);
    const el = document.querySelector<HTMLElement>('[data-va-rail]');
    if (el) ro.observe(el);
    return () => ro.disconnect();
  }, [railCollapsed]);

  // expose attributes so the page can *measure* the rail
  useEffect(() => {
    const el = document.querySelector<HTMLElement>('[data-va-rail]');
    if (el) el.setAttribute('data-va-collapsed', railCollapsed ? 'true' : 'false');
  }, [railCollapsed]);
  ...
  const expandBtnLeft = 'calc(var(--app-sidebar-w, 248px) - 1px)';
  ...
  return (
    <div ref={scopeRef} className={SCOPE}>
      ...
      {railCollapsed && (
        <button
          aria-label="Expand assistants"
          onClick={() => { setRailCollapsed(false); window.dispatchEvent(new Event('voice:layout:ping')); }}
          className="hidden lg:grid place-items-center"
          style={{
            position: 'fixed',
            left: expandBtnLeft,
            top: 'calc(var(--app-header-h, 64px) + 8px)',
            width: 36, height: 36,
            borderRadius: 10,
      ...
      <aside
        ...
        style={{
          ...
          // REAL collapse in layout => width: 0
          width: railCollapsed ? '0px' : 'var(--va-rail-w, 360px)',
          height: 'calc(100vh - var(--app-header-h, 64px))',
          borderRight: railCollapsed ? 'none' : '1px solid var(--va-border)',
          background: railCollapsed ? 'transparent' : 'var(--va-sidebar)',
          boxShadow: railCollapsed ? 'none' : 'var(--va-shadow-side)',
          zIndex: 10,
          overflow: 'hidden',         // prevent children from poking out by 1px on Safari
          transition: 'width .18s ease' // smooth widen/shrink
        }}
      >
        {/* Header */}
        {!railCollapsed && (
          <div className="px-3 py-3 flex items-center justify-between" style={{ borderBottom:'1px solid var(--va-border)' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                title="Collapse assistants"
                className="btn btn--ghost"
                onClick={() => { setRailCollapsed(true); window.dispatchEvent(new Event('voice:layout:ping')); }}
              >
