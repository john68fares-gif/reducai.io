...
/* ---------- Modal shell (matches api-keys page look) ---------- */
function ModalShell({ children }:{ children:React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center px-4"
         style={{ background:'rgba(0,0,0,.60)' }}>
      <div className="w-full max-w-[720px] rounded-[24px] overflow-hidden" style={{
        background:'var(--panel)', border:'1px solid var(--border)', boxShadow:'var(--shadow-soft)'
      }}>
        {children}
      </div>
    </div>
...
function Row({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-[12px] px-3 py-2 flex items-center gap-2 group"
      style={{
        background: active ? 'var(--rail-selected-bg)' : 'transparent',
        border: active ? '1px solid var(--rail-selected-border)' : '1px solid transparent',
  ...
  return (
    <div className="assistant-rail px-3 py-3 h-full"
      ...
      <button
        ...
        style={{ height: 40, background: BTN_GREEN, color: '#00120a' }}
        onMouseEnter={(e)=>((e.currentTarget as HTMLButtonElement).style.background=BTN_GREEN_HOVER)}
        onMouseLeave={(e)=>((e.currentTarget as HTMLButtonElement).style.background=BTN_GREEN)}
      ...
      <div className="relative mb-3">
        <input
          ...
          style={{ background:'var(--rail-input-bg)', borderColor:'var(--rail-input-border)', color:'var(--rail-input-text)', boxShadow:'var(--rail-input-shadow, none)' }}
        />
      ...
      {/* List */}
      <div className="overflow-auto" style={{ maxHeight:'calc(100% - 84px)' }}>
      ...
      {/* Theme tokens so it looks exactly like the screenshots */}
      <style jsx>{`
        /* Light */
        :global(:root:not([data-theme="dark"])) .assistant-rail{
          --rail-input-bg: #fff;
          --rail-input-border: rgba(0,0,0,.12);
          --rail-input-text: #0f172a;
          --rail-input-muted: #64748b;
          --rail-input-shadow: 0 1px 2px rgba(0,0,0,.04);

          --rail-selected-bg: rgba(16,185,129,.10);
          --rail-selected-border: rgba(16,185,129,.35);
        ...
        :global([data-theme="dark"]) .assistant-rail{
          ...
          --rail-selected-bg: color-mix(in oklab, var(--brand) 16%, var(--sb-icon-bg, rgba(255,255,255,.04)));
          --rail-selected-border: color-mix(in oklab, var(--brand) 45%, var(--border));
