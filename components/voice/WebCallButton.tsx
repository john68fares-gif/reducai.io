...
type Props = {
  ...
  // optional local client ASR (so your words always show)
  clientASR?: 'off' | 'auto' | 'deepgram';
  deepgramKey?: string; // only if clientASR="deepgram"
};
...
async function attachProcessedAudio(
  ...
  // optional breathing (auto-gated)
  let breathStop: null | (()=>void) = null;
  if (breathing){
    const { node, stop } = createBreather(ac, breathingLevel);
    breathStop = stop;
    const breathGain = ac.createGain(); breathGain.gain.value = (breathingLevel ?? 0.08) * 0.9;
    ...
    const gate = ()=> {
      ...
      const target = loud > 0.08 ? 0.02 : ((breathingLevel ?? 0.08) * 0.9);
...
function startWebSpeechASR(opts: { lang?: string; onStart:()=>void; onDelta: (t: string)=>void; onFinal: (t: string)=>void }) {
  ...
  rec.onresult = (e: any) => {
    ...
    if (!hadAny && (interim || finalText)) { hadAny = true; opts.onStart(); }
    if (interim) opts.onDelta(interim);
    if (finalText) { opts.onFinal(finalText); hadAny = false; }
  };
...
export default function WebCallButton({
  ...
  // NEW: turn managers (fixes the “single bubble” problem)
  const userTurnIdRef = useRef<string | null>(null);
  const asstTurnIdRef = useRef<string | null>(null);
  ...
  // USER turn helpers
  const beginUserTurn = (forceId?:string) => {
    const id = forceId || newId('user');
    userTurnIdRef.current = id;
    upsert(id,'user',{ text:'', done:false, at:Date.now() });
    return id;
  };
  const appendUserDelta = (delta:string, id?:string) => {
    const useId = id || userTurnIdRef.current || beginUserTurn();
    upsert(useId,'user',(prev)=>({ text:(prev?.text||'') + delta, done:false }));
  };
  const endUserTurn = (id?:string) => {
    const useId = id || userTurnIdRef.current;
    if (useId) upsert(useId,'user',{ done:true });
    if (!id || id === userTurnIdRef.current) userTurnIdRef.current = null;
  };

  // ASSISTANT turn helpers
  const beginAsstTurn = (forceId?:string) => {
    const id = forceId || newId('assistant');
    asstTurnIdRef.current = id;
    upsert(id,'assistant',{ text:'', done:false, at:Date.now() });
    return id;
  };
  const appendAsstDelta = (delta:string, id?:string) => {
    const useId = id || asstTurnIdRef.current || beginAsstTurn(id);
    upsert(useId,'assistant',(prev)=>({ text:(prev?.text||'') + delta, done:false }));
  };
  const endAsstTurn = (id?:string) => {
    const useId = id || asstTurnIdRef.current;
    if (useId) upsert(useId,'assistant',{ done:true });
    if (!id || id === asstTurnIdRef.current) asstTurnIdRef.current = null;
  };

  // minimal mic VAD ducking
  ...
  async function startCall(){
    ...
    try{
      ...
      // 2.5) LOCAL ASR (optional) — open a new user bubble per utterance
      try {
        const lang = langToBCP47(languageHint);
        if (asrStopRef.current) { try { asrStopRef.current(); } catch {} asrStopRef.current = null; }

        const onStart = () => { if (!userTurnIdRef.current) beginUserTurn(); };
        const onDelta = (txt: string) => appendUserDelta(txt.replace(/\s+/g,' ').trim() ? ` ${txt}` : '');
        const onFinal = (txt: string) => { appendUserDelta(` ${txt}`); endUserTurn(); };

        if (clientASR === 'deepgram') {
          if (!deepgramKey) throw new Error('Missing deepgramKey');
          const stop = startDeepgramASR({ lang, deepgramKey, stream: mic, onStart, onDelta, onFinal });
          if (stop) asrStopRef.current = stop;
        } else if (clientASR === 'auto') {
          const stop = startWebSpeechASR({ lang, onStart, onDelta, onFinal });
          if (stop) asrStopRef.current = stop;
        }
      } catch {}
      ...
      const safeSend=(payload:any)=>{ if(dc && dc.readyState==='open'){ try{ dc.send(JSON.stringify(payload)); }catch{} } };
