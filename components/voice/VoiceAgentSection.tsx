...
export default function VoiceAgentSection() {
  ...
  const [creating, setCreating] = useState(false); // creation phase
  ...
  useEffect(() => {
    ...
    if (!list.length) {
      const seed: Assistant = {
        ...
        updatedAt: Date.now(),
        config: {
          model: { provider:'openai', model:'gpt-4o', firstMessageMode:'assistant_first', firstMessage:'Hello.', systemPrompt: '' /* start blank */ },
  ...
  // CREATE: real flow with loading + empty editor
  const addAssistant = async () => {
    setCreating(true);
    const id = `agent_${Math.random().toString(36).slice(2, 8)}`;
    // “empty” config to start from zero
    const a: Assistant = {
      id,
    ...
    // simulate a brief async op (could be an API call later)
  ...
  const submitGenerate = () => {
    if (!active) return;
    const preview = (active.config.model.systemPrompt || '') + (genText.trim() ? `\n\n[Refinements]\n- ${genText.trim()}` : '');
  ...
  return (
    <div className={SCOPE} style={{ background:'var(--bg)', color:'var(--text)' }}>
      {/* Fixed rail that touches main sidebar + header */}
      <aside
        className="hidden lg:flex shrink-0 flex-col overflow-hidden"
        style={{
          position:'fixed',
          left: 'var(--app-sidebar-w, 240px)',
      ...
      {/* Editor shifts right exactly the rail width */}
      <main
        className="min-w-0"
        style={{
          marginLeft: 'calc(var(--app-sidebar-w, 240px) + 312px)',
        }}
      >
        {/* top bar (touches header) */}
        <div className="px-6 py-4 flex items-center justify-between sticky"
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <input
              ...
              onBlur={() => updateActive(a => ({ ...a }))} // bump timestamp
          ...
          {creating ? (
            ...
            <div className="col-span-12">
              <div className="rounded-xl p-10 text-center"
                ...
                <div className="mx-auto w-10 h-10 rounded-full border-4 border-white/30 border-t-transparent animate-spin" />
                <div className="mt-4 text-sm opacity-80">Creating new assistant…</div>
              </div>
            </div>
          ) : (
            <>
        ...
        {showDelete && (
          <motion.div className="fixed inset-0 z-[9998] flex items-center justify-center p-4"
            ...
            <motion.div
              ...
              className="w-[92vw] max-w-[520px] rounded-2xl overflow-hidden"
              style={{ background:'var(--va-card)', border:'1px solid var(--va-border)', boxShadow:'var(--va-shadow-lg)' }}
            >
...
/* =============================================================================
   Scoped CSS (Movatif is global; we just align layout/controls)
============================================================================= */
function StyleBlock() {
  return (
    <style jsx global>{`
      .${SCOPE}{
        ...
        /* host app can set these to match its shell */
        --app-header-h: 64px;
        --app-sidebar-w: 240px;
      }
      :root:not([data-theme="dark"]) .${SCOPE}{
        --bg:#0b0c10; /* your page is dark by design in the screenshots */
      }
