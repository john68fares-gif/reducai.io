...
type Settings = {
  systemPrompt: string; // scheduling prompt
  language: string;     // e.g. "en-US"
  ttsVoice: string;     // e.g. "Polly.Joanna" or "alice"
  fromE164: string;     // selected imported number
  assistantId?: string; // widget test
  publicKey?: string;   // widget test
};
type TwilioCreds = { accountSid: string; authToken: string } | null;
type Banner = { kind: 'success' | 'error' | 'info'; message: string } | null;

type BotSummary = { id: string; name?: string; title?: string; language?: string; industry?: string; prompt?: string; questionFlow?: string[]; faq?: any[]; notes?: string; rawNotes?: string; additionalContext?: string };

/* --------------------------- utils/store -------------------------- */
const LS_SETTINGS_KEY = 'voice:settings:backup';
const CHATBOTS_KEY = 'chatbots'; // array of bots
...
/* -------- seed from a chosen chatbot (selector) -------- */
function buildBusinessContextFromBot(bot: BotSummary | null): string {
  if (!bot) return '';
  const parts = [
...
export default function VoiceAgentSection() {
  // available chatbots for selector
  ...
  // initial load: numbers + bots + settings
  useEffect(() => {
    // bots
    try {
  ...
  // when selected bot changes, seed prompt/language
  useEffect(() => {
    if (!selectedBotId) return;
    const bot = (bots || []).find(b => (b as any).id === selectedBotId) || null;
    const ctx = buildBusinessContextFromBot(bot);
    if (ctx) {
      setSettings((p)=> ({ ...p, systemPrompt: shapePromptForScheduling('', ctx), language: bot?.language || p.language }));
    }
  }, [selectedBotId]);
  ...
  async function onAttachClick() {
    ...
    try {
      setAttaching(true);
      const r = await fetch('/api/telephony/attach-number', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ phoneNumber: settings.fromE164, credentials: creds }) });
      const ct = r.headers.get('content-type') || '';
      if (!ct.includes('application/json')) throw new Error('Server did not return JSON.');
      const j = await r.json();
  ...
  return (
    <section className="px-6 py-8" style={{ maxWidth: 980, margin:'0 auto' }}>
      ...
      <div className="mb-6 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {bots.map((b:any)=> (
              <option key={b.id} value={b.id}>{b.name || b.title || b.id}</option>
            ))}
          </select>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="text-white/80 text-xs md:text-sm">Choose a build (AI) on the right, or create one first. Re-use imported Twilio creds, attach a number, test, and deploy.</div>
        <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <Section title="Number (Imported with Twilio)">
            <div className="grid gap-3">
              ...
              <p className="text-xs text-white/60">Uses your Twilio creds saved during import (never asks twice).</p>
            </div>
          </Section>
          ...
          <Section title="Quick Tests">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <div className="text-sm text-white/90 mb-2 flex items-center gap-2"><Volume2 className="w-4 h-4" /> Audio Ping</div>
              ...
              <div>
                <div className="text-sm text-white/90 mb-2 flex items-center gap-2"><WidgetIcon className="w-4 h-4" /> Browser Widget</div>
...
// pages/agents/create.tsx
'use client';

import React, { useEffect, useState } from 'react';

const CARD_CSS: React.CSSProperties = { background:'#101314', border:'1px solid rgba(255,255,255,.25)', borderRadius:20, boxShadow:'inset 0 0 22px rgba(0,0,0,.28), 0 10px 40px rgba(0,0,0,.35)' };
const BTN = '#59d9b3'; const BTN_H = '#54cfa9';

export default function CreateAgentPage(){
  const [name, setName] = useState('New Voice Agent');
  const [language, setLanguage] = useState('en-US');
  const [industry, setIndustry] = useState('Healthcare');

  useEffect(()=>{ document.title = 'Create Agent • reduc.ai'; },[]);
  ...
  function addAgent(){
    try {
      ...
      const bot = { id, name, language, industry, notes: '' };
      const next = Array.isArray(arr) ? [...arr, bot] : [bot];
  ...
  return (
    <main className="px-6 py-10" style={{ maxWidth: 820, margin:'0 auto' }}>
      <h1 className="text-2xl font-semibold mb-4">Create Agent</h1>
      <div className="p-6 grid gap-4" style={CARD_CSS}>
        <label className="text-xs text-white/70">Agent Name
          <input value={name} onChange={(e)=>setName(e.target.value)} className="mt-1 w-full rounded-[12px] border border-white/20 bg-black/30 px-3 h-[38px] text-sm outline-none focus:border-[#6af7d1] text-white" />
        </label>
        <label className="text-xs text-white/70">Language (BCP-47)
          <input value={language} onChange={(e)=>setLanguage(e.target.value)} placeholder="en-US" className="mt-1 w-full rounded-[12px] border border-white/20 bg-black/30 px-3 h-[38px] text-sm outline-none focus:border-[#6af7d1] text-white" />
        </label>
        <label className="text-xs text-white/70">Industry
          <input value={industry} onChange={(e)=>setIndustry(e.target.value)} className="mt-1 w-full rounded-[12px] border border-white/20 bg-black/30 px-3 h-[38px] text-sm outline-none focus:border-[#6af7d1] text-white" />
        </label>
        <button onClick={addAgent} className="inline-flex items-center justify-center gap-2 px-4 h-[42px] rounded-[14px] font-semibold select-none" style={{ background: BTN, color:'#fff' }} onMouseEnter={(e)=> (e.currentTarget as HTMLButtonElement).style.background = BTN_H} onMouseLeave={(e)=> (e.currentTarget as HTMLButtonElement).style.background = BTN}>Create</button>
      </div>
      <p className="text-xs text-white/60 mt-3">After creating, you'll be redirected to <code>/voice-agent</code> where you can select the new build from the dropdown.</p>
    </main>
  );
}
// components/voice/VoiceAgentSection.tsx
