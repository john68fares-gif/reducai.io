...
import {
  Bot, Code2, Play, MessagesSquare, Phone,
  Wand2, ChevronDown, ChevronUp, Gauge, Timer,
} from 'lucide-react';
import AssistantRail from '@/components/voice/AssistantRail'; // ← rail

/* ===== Theme tokens (aligned with api-keys.tsx look & feel) ===== */
const GREEN = '#10b981';
...
function LocalTokens() {
  return (
    <style>{`
      ...
      .btn {
        height:46px; padding:0 1rem; border-radius:14px; display:inline-flex; align-items:center; gap:.55rem;
        background:var(--card); color:var(--text); border:1px solid var(--border); font-weight:600; font-size:14px;
        transition:transform .06s ease;
      }
      .btn:hover { transform: translateY(-1px); }
      .btn-primary {
        height:46px; padding:0 1.1rem; border-radius:18px; display:inline-flex; align-items:center; gap:.6rem;
...
/* Tiny toggle used in Transcriber */
const Toggle = ({checked,onChange}:{checked:boolean; onChange:(v:boolean)=>void}) => (
  <button
    onClick={()=>onChange(!checked)}
    className="btn"
    style={{
      height:34, padding:'0 6px', borderRadius:999,
      background: checked ? 'rgba(16,185,129,.18)' : 'var(--card)',
      borderColor: checked ? GREEN : 'var(--border)',
      width:56, justifyContent:'flex-start'
    }}
    aria-pressed={checked}
  >
    <span
      style={{
        width:22, height:22, borderRadius:999, background: checked ? GREEN : 'rgba(255,255,255,.12)',
...
export default function VoiceAgentSection() {
  const [tab] = useState<'Model'|'Voice'|'Transcriber'|'Tools'|'Analysis'|'Advanced'|'Widget'>('Model');
    ...
    <div className="va w-full" style={{ background:'var(--bg)', color:'var(--text)' }}>
      ...
      <div className="grid w-full" style={{ gridTemplateColumns:'312px 1fr' }}>
        ...
        <div className="px-4 md:px-6 lg:px-8 py-6 mx-auto w-full max-w-[1180px]">
          ...
          {/* Tabs row (visual only, like screenshot) */}
          <div className="mb-5 flex flex-wrap items-center gap-8">
            ...
            <div className="flex items-center gap-2">
              <span className="label-xs">Web</span>
            </div>
          </div>
          ...
          <div className="va-panel overflow-hidden">
            ...
            <div className="acc-head" onClick={()=>setOpenModel(v=>!v)}>
              <div className="acc-title">
                <span className="pill" style={{height:30}}><Gauge size={14}/></span>
                Model
              </div>
            ...
            <div className="acc-head" onClick={()=>setOpenTranscriber(v=>!v)}>
              <div className="acc-title">
                <span className="pill" style={{height:30}}><Timer size={14}/></span>
                Transcriber
              </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          style={{ width:'100%' }}
                        />
                        <div className="va-card px-3 py-2 rounded-md text-sm" style={{minWidth:54, textAlign:'center'}}>
