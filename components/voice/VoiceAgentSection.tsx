...
const Tokens = () => (
  <style jsx global>{`
    .va-scope{
      /* Spacing/Type rhythm (ChatGPT-ish) */
      --s-2: 8px; --s-3: 12px; --s-4: 16px; --s-6: 24px; --s-8: 32px;
      --radius-outer: 14px;   /* outer cards (less rounded) */
      ...
      /* Dark app shell */
      --vs-card: #101314;
      --vs-border: rgba(255,255,255,.04); /* almost invisible, “0px” feel */
      ...
      --vs-input-shadow: 0 16px 40px rgba(0,0,0,.46), 0 0 0 1px rgba(89,217,179,.05);
    ...
    :root:not([data-theme="dark"]) .va-scope{
      ...
      --vs-input-shadow: 0 16px 40px rgba(0,0,0,.12), 0 0 0 1px rgba(0,0,0,.03);
...
type AgentData = {
  ...
  /* Voice (UI only for now; wire later to OpenAI/ElevenLabs) */
  voiceProvider: 'openai' | 'elevenlabs';
  voiceName: string;  // e.g., 'alloy' (OpenAI) or your ElevenLabs voice key
  voiceId?: string;   // for ElevenLabs later
  ...
  /* Derived voice key (helper to map to TTS later) */
  voiceKey: string;
...
async function apiSave(agentId: string, payload: AgentData) {
  const r = await fetch(`/api/voice/agent/${agentId}/save`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  if (!r.ok) throw new Error('Save failed'); return r.json();
}
async function apiPublish(agentId: string) { const r = await fetch(`/api/voice/agent/${agentId}/publish`, { method: 'POST' }); if (!r.ok) throw new Error('Publish failed'); return r.json(); }
async function apiCallTest(agentId: string) { const r = await fetch(`/api/voice/agent/${agentId}/call-test`, { method: 'POST' }); if (!r.ok) throw new Error('Test call failed'); return r.json(); }
...
const asrModelsFor = (asr: string): Opt[] =>
  asr === 'deepgram' ? [{ value: 'Nova 2', label: 'Nova 2' }, { value: 'Nova', label: 'Nova' }] : [{ value: 'coming', label: 'Models coming soon', disabled: true }];
...
const FieldShell = ({ label, children, error, boxed = true }: {
  ...
  return (
    <div>
      ...
      {boxed ? (
        <div className="px-3 py-[10px] rounded-[var(--radius-inner)]" style={{ background: 'var(--vs-input-bg)', border: `1px solid ${borderBase}`, boxShadow: 'var(--vs-input-shadow)' }}>
          {children}
        </div>
      ) : children}
...
/* Solid, searchable portal dropdown */
    ...
    <>
      ...
      {open && rect
        ? createPortal(
            ...
            >
              <div className="flex items-center gap-2 mb-3 px-2 py-2 rounded-[10px]" style={{ background: 'var(--vs-input-bg)', border: '1px solid var(--vs-input-border)', boxShadow: 'var(--vs-input-shadow)', color: 'var(--text)' }}>
...
export default function VoiceAgentSection() {
  ...
  useEffect(() => { setData((prev) => ({ ...prev, voiceKey: voiceKeyFor(prev.asrLang, prev.asrDialect) })); }, [data.asrLang, data.asrDialect]); // eslint-disable-line
      ...
      <div className="grid w-full pr-[1px]" style={{ gridTemplateColumns: '260px 1fr' }}>
        ...
        <div className="px-3 md:px-5 lg:px-6 py-5 mx-auto w-full max-w-[1160px]" style={{ fontSize: 'var(--fz-body)', lineHeight: 'var(--lh-body)' }}>
          ...
          <div className="grid gap-[var(--s-4)] md:grid-cols-2 mb-[var(--s-6)]">
            <div className="relative p-[var(--s-4)] rounded-[12px] va-pop" style={{ background: 'var(--vs-card)', border: '1px solid var(--vs-border)', boxShadow: 'var(--card-shadow-outer)' }}>
          ...
          <Section title="Model" icon={<Gauge className="w-4 h-4" style={{ color: CTA }} />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--s-4)]">
              ...
              <FieldShell label="Model" boxed={false}>
                <StyledSelect value={data.model} onChange={set('model')} options={modelOptsFor(data.provider)} />
              </FieldShell>
            </div>
          ...
          <Section title="Voice" icon={<Volume2 className="w-4 h-4" style={{ color: CTA }} />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--s-4)]">
              <FieldShell label="Voice Provider" boxed={false}>
                <StyledSelect value={data.voiceProvider} onChange={(v) => set('voiceProvider')(v as AgentData['voiceProvider'])} options={voiceProviders} />
              </FieldShell>

              <FieldShell label="Voice" boxed={false}>
                <StyledSelect value={data.voiceName} onChange={set('voiceName')} options={openAIVoices} />
              </FieldShell>
            </div>
            ...
            <div className="mt-[var(--s-4)] text-xs" style={{ color: 'var(--text-muted)' }}>
              ...
              (You can swap to ElevenLabs later and map <span style={{ color: 'var(--text)' }}>voiceKey</span> to a voice ID.)
            </div>
          </Section>
          ...
          <Section title="Transcriber" icon={<Timer className="w-4 h-4" style={{ color: CTA }} />}>
              ...
              <FieldShell label="Language" boxed={false}>
                <StyledSelect
                  ...
                  onChange={(v) => {
                    ...
                    set('asrLang')(lang);
                    if (lang !== 'en') set('asrDialect')('standard'); // reset if not English
                  }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--s-4)] mt-[var(--s-4)]">
              <FieldShell label="Dialect" boxed={false}>
                <StyledSelect value={data.asrDialect} onChange={(v) => set('asrDialect')(v as AgentData['asrDialect'])} options={dialectsFor(data.asrLang)} />
              </FieldShell>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[var(--s-4)] mt-[var(--s-4)]">
              <FieldShell label="Confidence Threshold" boxed>
                <div className="flex items-center gap-[var(--s-3)]">
                  <input type="range" min={0} max={1} step={0.01} value={data.confidence} onChange={(e) => set('confidence')(parseFloat(e.target.value))} style={{ width: '100%' }} />
                  <div className="px-2.5 py-1.5 rounded-md text-xs" style={{ background: 'var(--vs-input-bg)', border: '1px solid var(--vs-input-border)', boxShadow: 'var(--vs-input-shadow)', minWidth: 46, textAlign: 'center', color: 'var(--text)' }}>
...
/* ───────────────── Section card (titles OUTSIDE; animated collapse; no unmount) ───────────────── */
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    if (open) {
      const ph = el.scrollHeight; setH(ph);
      const t = setTimeout(() => setH('auto'), 200); return () => clearTimeout(t);
    } else {
      const ph = el.scrollHeight; setH(ph);
    ...
    <div className="mb-[var(--s-6)]">
      ...
      <div className="w-full flex items-center justify-between mb-[var(--s-3)]">
        <span className="inline-flex items-center gap-[var(--s-3)]" style={{ color: 'var(--text)' }}>
          <span className="inline-grid place-items-center w-7 h-7 rounded-full" style={{ background: 'rgba(89,217,179,.12)', boxShadow: '0 0 0 1px rgba(89,217,179,.14) inset, 0 10px 24px rgba(89,217,179,.10)' }}>
            {icon}
          </span>
          <span className="font-semibold" style={{ fontSize: 'var(--fz-title)' }}>{title}</span>
        </span>
        <button onClick={() => setOpen((v) => !v)} className="px-2 py-1 rounded-md" style={{ background: 'transparent', color: 'var(--text-muted)' }}>
          {open ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
      </div>

      {/* Card with outside shadow + almost-zero border */}
      <div className="rounded-[var(--radius-outer)]" style={{ background: 'var(--vs-card)', border: '1px solid var(--vs-border)', boxShadow: 'var(--card-shadow-outer)', overflow: 'hidden' }}>
        <div
          ref={wrapRef}
        ...
        >
          <div className="relative p-[var(--s-6)]">
            <div aria-hidden className="pointer-events-none absolute -top-[28%] -left-[28%] w-[60%] h-[60%] rounded-full" style={{ background: 'radial-gradient(circle, var(--vs-ring) 0%, transparent 70%)', filter: 'blur(38px)' }} />
            {children}
          </div>
        </div>
      </div>
