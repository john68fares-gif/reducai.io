...
/* ------------------------------------------------------------------ */
/*  Reusable InlineSelect (compact; with portal dropdown)              */
/* ------------------------------------------------------------------ */
...
export default function VoiceAgentSection() {
  ...
  const [nums, setNums] = useState<NumberItem[] | any>([]);
  ...
  async function refreshNumbers() {
    try {
      ...
      setMsg('Could not refresh numbers (API).');
    }
  }
  ...
  function onPickBot(id: string) {
    ...
    if (bot) {
      const prompt = buildRawFromBot(bot);
      setSettings((p) => ({ ...p, systemPrompt: prompt }));
      setMsg(`Imported prompt from “${bot.name}”.`);
      setActiveTab('model');
      setOpenModelB(true);
    }
  }
  ...
  async function improveForVoice() {
    try {
      ...
      const ct = r.headers.get('content-type') || '';
      if (!ct.includes('application/json')) throw new Error('Improve failed (non-JSON).');
      const j = await r.json();
      if (!j?.ok) throw new Error(j?.error || 'Improve failed');
      setSettings((p) => ({ ...p, systemPrompt: j.data?.prompt || p.systemPrompt }));
      setOpenModelB(true);
      setMsg('Prompt re-shaped for a voice agent.');
    } catch (e: any) {
      setMsg(e?.message || 'Could not improve prompt.');
    }
  }
  ...
  async function attachNumber(agentId: string, phoneNumber: string) {
    ...
    setMsg(`Agent live at ${phoneNumber}`);
  }
  ...
  function mountWidget() {
    ...
    if (!settings.assistantId || !settings.publicKey) {
      setMsg('Enter assistant id and public key first.');
      return;
    }
    ...
    if (slot && slot.childElementCount === 0) {
      ...
      setMsg('Widget mounted. Click the floating button to talk.');
    }
  }

  const numsArr: NumberItem[] = Array.isArray(nums) ? nums : [];
  const numberOptions: Option[] = useMemo(
    () => numsArr.map((n) => ({
      value: n.e164 || '',
      label: (n.e164 ? n.e164 : n.id) + (n.label ? ` — ${n.label}` : ''),
    })),
    [numsArr]
  );

  const botOptions: Option[] = useMemo(
  ...
  if (loading) {
    return (
      <div className="relative p-8" style={{ ...FRAME, overflow: 'visible', maxWidth: 860, margin: '0 auto' }}>
    ...
    <div className="relative p-6 md:p-8" style={{ ...FRAME, overflow: 'visible', maxWidth: 860, margin: '0 auto' }}>
      ...
      {activeTab === 'model' && (
        <div className="grid grid-cols-1 gap-6">
          <Section
            title="Import from Build"
            icon={<Cog className="w-5 h-5 text-[#6af7d1]" />}
            open={openModelA}
            onToggle={() => setOpenModelA(v=>!v)}
          >
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 ml-auto">
                <GreenButton className="w-[190px]" onClick={improveForVoice}>
                  <Wand2 className="w-4 h-4 text-white" />
                  Improve for Voice
                </GreenButton>
                <GreenButton className="w-[150px]" onClick={()=>setOpenModelB(true)}>
                  Preview / Edit
                </GreenButton>
              </div>
            </div>
          ...
          <Section
            ...
            icon={<AdvancedIcon className="w-5 h-5 text-[#6af7d1]" />}
            open={openModelB}
            onToggle={() => setOpenModelB(v=>!v)}
          >
            <textarea
              rows={14}
              ...
              placeholder="Your assistant prompt…"
              style={{ minHeight: 220 }}
            />
          </Section>
        </div>
      ...
      {activeTab === 'voice' && (
        <div className="grid grid-cols-1 gap-6">
          <Section
            ...
            icon={<Mic className="w-5 h-5 text-[#6af7d1]" />}
            open={openVoiceA}
            onToggle={() => setOpenVoiceA(v=>!v)}
          >
          ...
          <Section
            title="Assigned Number (imported only)"
            icon={<PhoneIcon className="w-5 h-5 text-[#6af7d1]" />}
            open={openVoiceB}
            onToggle={() => setOpenVoiceB(v=>!v)}
          >
            <div className="flex flex-wrap items-center gap-3">
              ...
              <GreenButton className="ml-auto w-[220px]" onClick={refreshNumbers}>
                ...
                Refresh Imported Numbers
              </GreenButton>

              <GreenButton
                className="w-[220px]"
                onClick={onAttachClick}
                disabled={!selectedBotId || !settings.fromE164 || attaching}
              >
            ...
            <p className="text-xs text-white/60 mt-2">
              Uses your selected Build as <code>agentId</code>. Sets the Twilio Voice URL automatically and routes calls.
            </p>
          </Section>
        </div>
      )}
      ...
      {activeTab === 'transcriber' && (
        <div className="grid grid-cols-1 gap-6">
          <div style={CARD} className="p-5 text-white/80">
            Your ASR language is set in the <b>Voice</b> tab. More transcriber options can live here later.
          </div>
        </div>
      )}
      ...
      {activeTab === 'tools' && (
        <div className="grid grid-cols-1 gap-6">
          <div style={CARD} className="p-5 text-white/80">
            Add integrations, actions, or fetchers here as your app grows.
          </div>
        </div>
      )}
      ...
      {activeTab === 'analysis' && (
        <div className="grid grid-cols-1 gap-6">
          <div style={CARD} className="p-5 text-white/80">
            Drop analytics and insights here (call outcomes, intents, average handle time, etc.).
          </div>
        </div>
      )}
      ...
      {activeTab === 'advanced' && (
        <div className="grid grid-cols-1 gap-6">
          <div style={CARD} className="p-5 text-white/80">
            Advanced options can be added here later.
          </div>
        </div>
      )}
      ...
      {activeTab === 'widget' && (
        <div className="grid grid-cols-1 gap-6">
          <Section
            title="Browser Test (no phone)"
            icon={<WidgetIcon className="w-5 h-5 text-[#6af7d1]" />}
            open={openWidgetA}
            onToggle={() => setOpenWidgetA(v=>!v)}
          >
