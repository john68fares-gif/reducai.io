                ...
                >
                  ...
                  <button onClick={() => {
                    ...
                    if (newName && newName.trim()) {
                      updateActive((cur) => cur.id === a.id ? { ...cur, name: newName.trim() } : cur);
                    }
                  }} className="ml-2 btn--ghost px-2"><Edit3 className="w-3.5 h-3.5 icon" /></button>
                  <button onClick={() => {
                    if (confirm(`Delete assistant "${a.name}"?`)) removeAssistant(a.id);
                  }} className="ml-1 btn--ghost px-2"><Trash2 className="w-3.5 h-3.5 icon" /></button>
                </button>
              ))}
            </div>
          </div>
