...
/* =============================================================================
   CONSTANTS / TOKENS (match API Keys look)
============================================================================= */
...
function diffText(oldStr: string, newStr: string) {
  ...
  while(i<o.length&&j<n.length){ if(o[i]===n[j]){ out.push(o[i]); i++; j++; } else if(dp[i+1][j]>=dp[i][j+1]){ out.push(`<del>${o[i++]}</del>`);} else { out.push(`<ins>${n[j++]}</ins>`);} }
  while(i<o.length) out.push(`<del>${o[i++]}</del>`); while(j<n.length) out.push(`<ins>${n[j++]}</ins>`);
  return out.join(' ');
}
...
function Select({ value, items, onChange, placeholder, leftIcon }:{
  ...
  useEffect(()=>{ if(!open) return; const on=(e:MouseEvent)=>{ if(btn.current?.contains(e.target as Node)) return; if(portal.current?.contains(e.target as Node)) return; setOpen(false);}; window.addEventListener('mousedown',on); return ()=>window.removeEventListener('mousedown',on);},[open]);
  const filtered = items.filter(i=>i.label.toLowerCase().includes(q.toLowerCase())); const sel = items.find(i=>i.value===value)||null;
  return (
    <>
      <button ref={btn} type="button" onClick={()=>setOpen(v=>!v)} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-[15px]"
...
/* =============================================================================
   MODALS
============================================================================= */
function LoadingOverlay({ text='Creating…' }:{ text?:string }){
  return (
    <motion.div className="fixed inset-0 z-[1000] flex items-center justify-center px-3"
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ background:'rgba(0,0,0,.55)' }}>
      <motion.div initial={{ scale:.96, opacity:.9 }} animate={{ scale:1, opacity:1 }} className="rounded-2xl p-6"
        style={{ background:'var(--va-card)', border:'1px solid var(--va-border)', boxShadow:'var(--va-shadow-lg)' }}>
        <div className="flex items-center gap-3">
          <span className="w-5 h-5 rounded-full border-2 border-white/50 border-t-transparent animate-spin" />
...
function DeleteModal({ open, onCancel, onConfirm }:{ open:boolean; onCancel:()=>void; onConfirm:()=>void }){
  if(!open) return null;
  return (
    <motion.div className="fixed inset-0 z-[1000] flex items-center justify-center px-4"
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ background:'rgba(0,0,0,.55)' }}>
      <motion.div initial={{ y:8, opacity:0, scale:.98 }} animate={{ y:0, opacity:1, scale:1 }} exit={{ y:8, opacity:0, scale:.985 }}
        ...
        <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom:'1px solid var(--va-border)' }}>
          <div className="flex items-center gap-2 text-sm font-semibold" style={{ color:'var(--text)' }}>
            <Trash2 className="w-4 h-4" /> Delete Assistant
          </div>
          <button onClick={onCancel} className="p-2 rounded-full hover:opacity-80"><X className="w-4 h-4 icon" /></button>
        </div>
        <div className="px-5 py-4 text-sm" style={{ color:'var(--text-muted)' }}>
          This action cannot be undone. Do you really want to delete this assistant?
        </div>
...
export default function VoiceAgentSection() {
  ...
  const updateActive = (mut: (a: Assistant) => Assistant) => {
    ...
    const list = (readLS<Assistant[]>(LS_LIST) || []).map(x => x.id === next.id ? { ...x, name: next.name, folder: next.folder, updatedAt: next.updatedAt } : x);
    writeLS(LS_LIST, list); setAssistants(list);
  };

  // CREATE with loading overlay -> empty config
  const addAssistant = async () => {
    ...
    writeLS(ak(id), empty);
    const list = [...assistants, empty]; writeLS(LS_LIST, list);
    setAssistants(list); setActiveId(id);
    setCreating(false);
  };

  const confirmDelete = (id: string) => setShowDelete(id);
  const performDelete = () => {
    const id = showDelete!;
  ...
  const acceptChanges = () => { if (!active || !pending) return; updateActive(a => ({ ...a, config:{ ...a.config, model:{ ...a.config.model, systemPrompt: pending.preview }}})); setPending(null); };
  ...
  if (!active) {
    return (
      <div className={SCOPE} style={{ color:'var(--text)' }}>
        <ShellLayout>
          <div className="px-6 py-12 text-sm opacity-70">Create your first assistant.</div>
        </ShellLayout>
        <StyleBlock />
      </div>
    );
  ...
  return (
    <div className={SCOPE} style={{ background:'var(--bg)', color:'var(--text)' }}>
      <ShellLayout>
        {/* ================= FIXED ASSISTANTS RAIL (FLUSH to main sidebar & header) ================ */}
        <aside
          className="hidden lg:flex shrink-0 flex-col"
          style={{
            position:'fixed',
            top:'var(--app-header-h, 64px)',
            left:'calc(var(--app-sidebar-w, 240px) + var(--app-content-gap, 0px))',
            width:312,
        ...
        >
          ...
          <div className="p-3 min-h-0">
            ...
            <div className="mt-4 space-y-2">
              {visible.map(a => (
                <button key={a.id} onClick={()=> setActiveId(a.id)} className="w-full text-left rounded-xl p-3 flex items-center justify-between"
                  style={{ background: a.id===activeId ? 'color-mix(in oklab, var(--accent) 10%, transparent)' : 'var(--va-card)',
                  ...
                  <div className="min-w-0">
                    <div className="font-medium truncate flex items-center gap-2"><Bot className="w-4 h-4 icon" /><span className="truncate">{a.name}</span></div>
        ...
        <main className="min-w-0"
          style={{
            // push main content exactly by the rail width + main app sidebar + optional gap
            marginLeft: 'calc(var(--app-sidebar-w, 240px) + var(--app-content-gap, 0px) + 312px)',
          }}
        >
          {/* top bar */}
          <div className="px-6 py-4 flex items-center justify-between sticky"
            ...
            <div className="flex items-center gap-2">
              <button onClick={()=> navigator.clipboard.writeText(active.config.model.systemPrompt||'').catch(()=>{})} className="btn--ghost">
                <Copy className="w-4 h-4 icon" /> Copy Prompt
              </button>
              <button onClick={()=> setShowDelete(active.id)} className="btn--danger">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={()=> updateActive(a => ({ ...a, config:{ ...a.config, model:{ ...a.config.model, systemPrompt: BASE_PROMPT } } }))}
                            className="btn--ghost"><RefreshCw className="w-4 h-4 icon" /> Reset</button>
            ...
            <Section title="Voice" icon={<Mic2 className="w-4 h-4 icon" />}>
              ...
              <div className="mt-3">
                <button
                  ...
                  className="btn--ghost"><UploadCloud className="w-4 h-4 icon" /> Import from ElevenLabs</button>
              </div>
            </Section>
          ...
          <motion.div className="fixed inset-0 z-[999] flex items-center justify-center p-4"
            ...
            <motion.div initial={{ y:10, opacity:0, scale:.98 }} animate={{ y:0, opacity:1, scale:1 }} exit={{ y:8, opacity:0, scale:.985 }}
              ...
              <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom:'1px solid var(--va-border)' }}>
                ...
                <button onClick={()=>setGenOpen(false)} className="p-2 rounded-full hover:opacity-80"><X className="w-4 h-4 icon"/></button>
              </div>
              <div className="p-4">
      ...
      <AnimatePresence>
        {pending && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
            ...
            {typing && <div className="text-[12px] opacity-70 mb-3" style={{ fontFamily:'ui-monospace' }}>{typing}<span className="animate-pulse">▌</span></div>}
      ...
      {/* Create loading & Delete modal */}
      <AnimatePresence>{creating && <LoadingOverlay text="Creating assistant…"/>}</AnimatePresence>
      <AnimatePresence>{!!showDelete && <DeleteModal open={!!showDelete} onCancel={()=>setShowDelete(null)} onConfirm={performDelete}/>}</AnimatePresence>
...
/* =============================================================================
   Layout wrapper – keeps header sticky reference consistent
============================================================================= */
function ShellLayout({ children }:{ children:React.ReactNode }){
  return (
    <div>
      {/* This wrapper lets the fixed rail sit under the app header and next to the main sidebar */}
      <div>{children}</div>
    </div>
  );
}
