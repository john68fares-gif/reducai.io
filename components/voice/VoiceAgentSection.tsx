...
// Editor modal (the one I sent earlier)
...
const fadeUp: any = { initial: { opacity: 0, y: 14 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.22 } };
const liftOnHover = { whileHover: { y: -4, scale: 1.005 }, transition: { type: 'spring', stiffness: 220, damping: 18 } };
...
export default function VoiceAgentSection() {
  ...
  // keep an in-memory ref of the item being edited
  useEffect(() => {
    if (!editingId) { setEditingBot(null); return; }
    const bot = agents.find(a => a.id === editingId) || null;
    setEditingBot(bot);
  }, [editingId, agents]);
  ...
  if (mode === 'wizard') {
    return (
      <section className="w-full">
        {/* wider wrapper to make step cards wider; stays centered */}
