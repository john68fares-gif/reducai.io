...
/* =============================================================================
   Small UI atoms (kept compact) + style
============================================================================= */
...
function Select({ value, items, onChange, placeholder }: {
  ...
  useEffect(()=>{ if(!open) return; const r=btn.current?.getBoundingClientRect(); if(!r) return; const up=r.bottom+320>window.innerHeight; setRect({top:up?r.top:r.bottom,left:r.left,width:r.width,up}); },[open]);
  useEffect(()=>{ if(!open) return; const on=(e:MouseEvent)=>{ if(btn.current?.contains(e.target as Node) || (document.querySelector('.select-portal')?.contains(e.target as Node))) return; setOpen(false); }; window.addEventListener('mousedown', on); return ()=>window.removeEventListener('mousedown', on); },[open]);
  return (
    <>
      <button ref={btn} type="button" onClick={()=>setOpen(v=>!v)} className="w-full flex items-center gap-3 px-3 py-3 rounded-2xl text-[15px]"
        ...
        <span className="ml-auto" /><svg width="16" height="16" viewBox="0 0 24 24" className="opacity-70"><path fill="currentColor" d="M7 10l5 5 5-5z"/></svg>
      </button>
      {open && rect && (
        <div className="select-portal fixed z-[9999] p-3 rounded-xl"
          ...
          <div className="flex items-center gap-2 mb-3 px-2 py-2 rounded-lg" style={{ background:'var(--va-input-bg)', border:'1px solid var(--va-input-border)' }}>
          ...
          <div className="max-h-72 overflow-y-auto pr-1" style={{ scrollbarWidth:'thin' }}>
            {filtered.map(it=>(
              <button key={it.value} onClick={()=>{ onChange(it.value); setOpen(false); }} className="w-full text-left px-3 py-2 rounded-[10px]" style={{ color:'var(--text)' }}>
                {it.label}
              </button>
            ))}
...
type CallLog = { id:string; assistantId:string; assistantName:string; startedAt:number; endedAt?:number; endedReason?:string; type:'Web'; assistantPhoneNumber?:string; transcript:TranscriptTurn[]; };
...
const LS_LIST='voice:assistants.v1'; const LS_CALLS='voice:calls.v1'; const LS_ROUTES='voice:phoneRoutes.v1';
...
/* =============================================================================
   Scoped storage buckets (Step 2 parity)
============================================================================= */
...
/* =============================================================================
   Prompt + Speech (with Audio unlock fix)
============================================================================= */
...
/** Shared audio + AudioContext unlock to beat autoplay policies */
const sharedAudio = typeof window!=='undefined' ? new Audio() : null;
let audioUnlocked = false;
async function unlockAudioOnce(){
  if (audioUnlocked || !sharedAudio) return;
  try {
    // Attempt to resume any audio context if present
    // (We don't create one to avoid extra deps; triggering a .play() on silence also helps)
    sharedAudio.muted = true;
    sharedAudio.src = 'data:audio/mp3;base64,//uQZAAAAAAAAAAAAAAAAAAAA'; // tiny silent mp3 header
...
async function ttsElevenLabs(text:string, voiceId:string){
  ...
  try {
    ...
    // if autoplay hit again, try unlock and retry once
...
async function speak(text:string, provider:VoiceProvider, voiceId:string){
  try {
    if(provider==='elevenlabs'){ await ttsElevenLabs(text, voiceId); return; }
  } catch (e) { console.warn('TTS failed, using fallback', e); }
...
export default function VoiceAgentSection(){
  /* ----- load OpenAI keys + phones from scoped storage (like Step 2) ----- */
  ...
  // Default assistant key id after registries load
  useEffect(()=>{
  ...
  const addAssistant = ()=>{
    ...
    writeLS(ak(id), a);
    const list=[...(readLS<Assistant[]>(LS_LIST)||[]), a]; writeLS(LS_LIST, list);
  ...
  const testVoice=async()=>{ if(!active||!pendingVoiceId) return; await unlockAudioOnce(); await speak('This is a quick preview of the selected voice.', active.config.voice.provider, pendingVoiceId); };
  ...
  async function chatLLM(userText:string):Promise<string>{
    ...
    if (!openaiKey) return 'No OpenAI API key found in scoped storage. Please add one on the API Keys page.';
  ...
  async function startCall(){
    ...
    await unlockAudioOnce(); // <<< critical for autoplay/AudioContext
  ...
  /* =============================================================================
     Render (kept to your sidebar look/feel)
  ============================================================================= */
  return (
    <div className={SCOPE} style={{ background:'var(--bg)', color:'var(--text)' }}>
      {/* LEFT RAIL (fixed) */}
      <aside className="hidden lg:flex flex-col"
        style={{ position:'fixed', left:'calc(var(--app-sidebar-w, 248px) - 1px)', top:'var(--app-header-h, 64px)', width:'360px',
                 height:'calc(100vh - var(--app-header-h, 64px))', borderRight:'1px solid var(--va-border)', background:'var(--va-card)'}}>
            ...
            {visible.map(a=>{
              ...
              return (
                <div key={a.id} className="w-full rounded-xl p-3"
                  style={{ background:isActive?'color-mix(in oklab, var(--accent) 10%, transparent)':'var(--va-card)', border:`1px solid ${isActive?'color-mix(in oklab, var(--accent) 35%, var(--va-border))':'var(--va-border)'}`}}>
      ...
      <div className="va-main" style={{ marginLeft:'calc(var(--app-sidebar-w, 248px) + 360px)', padding:'24px 40px 88px' }}>
        ...
        <div className="px-2 pb-3 flex items-center justify-between sticky" style={{ top:'calc(var(--app-header-h, 64px) + 8px)', zIndex:2 }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={()=> alert(loaded ? (apiKeys.length ? 'Keys loaded.' : (loadErr || 'No API keys found.')) : 'Loading keys…')} className="btn btn--ghost">Check Keys</button>
        ...
        <div className="mx-auto grid grid-cols-12 gap-10" style={{ maxWidth:'min(2400px, 98vw)' }}>
            ...
            <div className="grid gap-6" style={{ gridTemplateColumns:'repeat(4, minmax(280px, 1fr))' }}>
              ...
              <Field label="Model">
                <Select value={active.config.model.model} onChange={(v)=> updateActive(a=>({...a, config:{...a.config, model:{...a.config.model, model:v as ModelId}}}))}
                  items={[{ value:'gpt-4o', label:'GPT-4o' },{ value:'gpt-4o-mini', label:'GPT-4o mini' },{ value:'gpt-4.1', label:'GPT-4.1' },{ value:'gpt-3.5-turbo', label:'GPT-3.5 Turbo' }]} />
              </Field>
              <Field label="OpenAI Key (scoped)">
                <Select
              ...
              <Field label="Temperature">
                <input type="number" step={0.1} min={0} max={1} value={active.config.model.temperature ?? 0.5}
                  ...
                  className="w-full rounded-2xl px-3 py-3 text-[15px] outline-none" style={{ background:'var(--va-input-bg)', border:'1px solid var(--va-input-border)' }}/>
              </Field>
            </div>
          ...
          <Section title="Call Assistant (Web test)" icon={<AudioLines className="w-4 h-4" />}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="text-xs opacity-70">Brain = your OpenAI key (scoped). Voice = ElevenLabs (US).</div>
            </div>
      ...
      {/* Delete modal */}
      <DeleteModal open={!!deleting} name={deleting?.name||''} onCancel={()=>setDeleting(null)} onConfirm={()=>{ if(deleting){ removeAssistant(deleting.id); setDeleting(null); } }} />
