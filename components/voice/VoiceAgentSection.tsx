...
function readVoiceAgents(): Agent[] {
  const all = safeParse<any[]>(typeof window !== 'undefined' ? localStorage.getItem('chatbots') : null, []);
  const onlyVoice = Array.isArray(all) ? all.filter(b => (b?.type || 'voice') === 'voice') : [];
...
> {
  ...
  componentDidCatch(err: Error) { console.error('[Editor crashed]', err); }
  render() {
    if (this.state.err) {
      return (
        <div className="min-h-screen bg-[#0b0c10] text-white font-movatif">
          <div className="w-full max-w-[1200px] mx-auto px-6 pt-16 pb-24">
            <div className="rounded-2xl border p-6" style={{ borderColor:'rgba(255,255,255,0.16)', background:'#101314' }}>
              <div className="text-lg font-semibold mb-2">Editor error</div>
              <div className="text-white/70 text-sm">
                {this.state.err.message || 'Unexpected client error.'}
              </div>
              <button
                onClick={this.props.onBack}
                className="mt-5 inline-flex items-center gap-2 rounded-[12px] border border-white/15 px-4 py-2 hover:bg-white/10"
              >
                ← Back to Agents
              </button>
            </div>
...
export default function VoiceAgentSection() {
  ...
  useEffect(() => {
    const read = () => setAgents(readVoiceAgents());
    read();
  ...
  const filtered = useMemo(() => {
    ...
    return agents.filter(
      (a) => a.name.toLowerCase().includes(q) || (a.fromE164 || '').toLowerCase().includes(q)
    );
  }, [agents, query]);

  const del = (id: string) => {
    try {
      const all = safeParse<any[]>(localStorage.getItem('chatbots'), []);
  ...
  if (mode === 'edit' && editingId) {
    return (
      <EditorErrorBoundary onBack={() => { setEditingId(null); setMode('gallery'); }}>
        <VoiceAssistantEditor
          id={editingId}
          onExit={() => { setEditingId(null); setMode('gallery'); }}
          onSaved={() => setAgents(readVoiceAgents())}
        />
      </EditorErrorBoundary>
    );
  }
      ...
      <section className="w-full">
        <div className="w-full max-w-[1880px] mx-auto px-6 2xl:px-14 pt-8 pb-28">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}
            ...
            <button onClick={exitWizard} className="inline-flex items-center gap-2 rounded-[12px] border border-white/15 px-4 py-2 hover:bg-white/10">
    ...
    <section className="w-full">
      <div className="w-full max-w-[1640px] mx-auto px-6 2xl:px-12 pt-8 pb-24">
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}
          ...
          <button onClick={startWizard} className="px-4 py-2 rounded-[10px] bg-[#00ffc2] text-black font-semibold shadow-[0_0_10px_rgba(106,247,209,0.28)] hover:brightness-110 transition">
            Create Voice Agent
          </button>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }} className="mb-8">
          <input
            value={query}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-7">
          <CardShell><CreateCard onClick={startWizard} /></CardShell>
...
function CreateCard({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="group relative h-[320px] rounded-[16px] p-7 flex flex-col items-center justify-center w-full active:scale-[0.995] transition-all">
      <div className="w-20 h-20 rounded-full flex items-center justify-center mb-5"
        style={{ background: 'rgba(0,0,0,0.18)', border: '1px dashed rgba(106,247,209,0.35)', boxShadow: 'inset 0 0 18px rgba(0,0,0,0.45), inset 0 0 6px rgba(106,247,209,0.06)' }}>
...
function AgentCard({
  agent, onDelete, onOpen, onEdit,
}: { agent: Agent; onDelete: () => void; onOpen: () => void; onEdit: () => void }) {
  return (
