...
export default function VoiceAgentSection() {
  ...
  async function attach() {
    ...
    try {
      ...
      setMsg(
        `✅ Attached ${phoneNumber} to "${agentId}"\n` +
          `Incoming webhook: ${window.location.origin}/api/voice/twilio/incoming`
      );
    } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gap: 12, maxWidth: 520 }}>
        <label>
          <div style={{ opacity: 0.8, marginBottom: 4 }}>Agent ID</div>
          <input
            value={agentId}
        ...
        {msg && <div style={{ whiteSpace: 'pre-wrap', opacity: 0.95 }}>{msg}</div>}

        <div style={{ marginTop: 8, opacity: 0.7 }}>
          Webhook (set in Twilio “A CALL COMES IN”):{' '}
          <code>/api/voice/twilio/incoming</code>
        </div>
      </div>
    </main>
  );
