...
/* ----------------------------- THEME (matches other steps) ----------------------------- */
const FRAME_BG = 'rgba(13,15,17,0.92)';
const CARD_BORDER = '1px solid rgba(106,247,209,0.18)';
const CARD_SHADOW = 'inset 0 0 22px rgba(0,0,0,0.28), 0 0 18px rgba(106,247,209,0.05)';
...
const modalFrame: React.CSSProperties = {
...
const modalHeaderBorder = { borderBottom: '1px solid rgba(255,255,255,0.4)' };
...
const LS_SIDEBAR = 'app:sidebarCollapsed';
...
function SidebarToggle() {
  ...
  useEffect(() => {
    try {
      ...
      // Add/remove a class your layout CSS can use to shrink/hide the sidebar
      document.body.classList.toggle('sidebar-collapsed', collapsed);
      // Optional: broadcast so a global Sidebar component can react
      window.dispatchEvent(new CustomEvent('ui:sidebar-toggle', { detail: { collapsed } }));
    } catch {}
  }, [collapsed]);
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-[24px] border border-white/15 px-3.5 py-2 hover:bg-white/10 text-sm"
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <label className="flex items-center gap-3 cursor-pointer select-none">
      <button type="button" onClick={() => onChange(!checked)} className={`w-12 h-7 rounded-full relative ${checked ? 'bg-emerald-400/80' : 'bg-white/10'} transition`}>
...
function ModalFrame({ open, onClose, title, children }: { open: boolean; onClose: () => void; title: string; children: React.ReactNode; }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="relative w-full max-w-[980px] max-h-[88vh] flex flex-col text-white font-movatif" style={modalFrame}>
        <div className="flex items-center justify-between px-6 py-4 rounded-t-[30px]" style={modalHeaderBorder}>
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-white/10" aria-label="Close"><X className="w-5 h-5 text-white" /></button>
        </div>
  ...
  return (
    <div className="min-h-screen w-full bg-[#0b0c10] text-white font-movatif">
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-8 pt-10 pb-24">
        ...
        >
          <div className="space-y-1">
            ...
            <div className="text-white/70 text-sm">Step 3 — Part A (core behavior)</div>
          </div>
        ...
        {/* 3-up grid with consistent spacing + subtle hover lift */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-7">
          ...
          <Box title="Information to Collect" subtitle="Drag to set ask order." icon={<ClipboardList className="w-4 h-4 text-[#6af7d1]" />}
               ...
               renderEdit={() => (<div style={cardStyle} className="p-5"><ChipList options={COLLECT_OPTIONS} value={s3.collect} onChange={v => set('collect', v)} reorderable /></div>)}
               saveBadge={saveBadge}>
      ...
      <ModalFrame open={previewOpen} onClose={() => setPreviewOpen(false)} title="Compiled System Prompt">
        <div style={cardStyle} className="p-5"><pre className="whitespace-pre-wrap text-sm leading-6">{compiled}</pre></div>
      </ModalFrame>
    </div>
  );
}
