...
/** A single list we’ll reuse for both Language and Dialect/Accent (per your request) */
const LANGUAGE_OPTS = [
  'English',
  'Spanish',
  'French',
...
export default function StepV1Basics({ onNext }: Props) {
  ...
  const [language, setLanguage] = useState<(typeof LANGUAGE_OPTS)[number]>('English');
  const [dialect, setDialect]   = useState<(typeof LANGUAGE_OPTS)[number]>('English');
  ...
  const errors = useMemo(() => {
    ...
    if (!dialect)          e.dialect = 'Please choose a dialect/accent.';
    return e;
  }, [name, industry, language, dialect]);
  ...
  return (
    <section className="relative voice-step1">
      {/* Header */}
      ...
      {/* Fields — NO outer rectangle/card wrapper */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Language (big dropdown) */}
        <FieldShell label="Language" error={errors.language}>
          <FancySelect
            value={language}
            options={LANGUAGE_OPTS as unknown as string[]}
            onChange={setLanguage}
          />
        </FieldShell>

        {/* Dialect / Accent (exact same dropdown as Language, per request) */}
        <FieldShell
          label={<>Dialect / Accent <span className="text-xs" style={{ color: 'var(--text-muted)' }}>(choose)</span></>}
      ...
      <div className="mt-8 flex justify-end">
        <button
          ...
          className="inline-flex items-center gap-2 px-8 h-[48px] rounded-[24px] font-semibold select-none transition will-change-transform disabled:opacity-60 disabled:cursor-not-allowed"
          style={{
      ...
      <style jsx global>{`
        ...
        [data-theme="dark"] .voice-step1__portal{
          --vs-menu-bg: #101314;
  ...
  return (
    <>
      <button
        ...
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-[16px] text-[15px] transition"
        style={{
          background: 'var(--vs-input-bg)',
      ...
      {open && rect && typeof document !== 'undefined'
        ? createPortal(
            <div
              ref={portalRef}
              className="voice-step1__portal fixed z-[9999] p-3"
              style={{
