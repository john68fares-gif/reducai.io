...
export default function Landing() {
  const handleSignUp = () => signIn('google', { callbackUrl: '/builder?onboard=1&mode=signup' });
  const handleSignIn = () => signIn('google', { callbackUrl: '/builder?mode=signin' });
    ...
    <main style={STYLES.page}>
      ...
      <header style={STYLES.nav}>
        ...
        <nav style={STYLES.navLinks}>
          ...
          {/* Auth buttons */}
          <button onClick={handleSignIn} style={STYLES.ctaGhost}>Sign in</button>
          <button onClick={handleSignUp} style={STYLES.ctaPrimary}>Sign up</button>
        </nav>
      </header>
...
const STYLES: Record<string, React.CSSProperties> = {
  ...
  grid: { position: "absolute", inset: 0,
    backgroundImage:"repeating-linear-gradient(0deg, rgba(255,255,255,0.04) 0 1px, transparent 1px 40px), repeating-linear-gradient(90deg, rgba(255,255,255,0.04) 0 1px, transparent 1px 40px)",
    maskImage: "radial-gradient(ellipse at 50% 0%, rgba(0,0,0,.9), transparent 65%)",
    animation: "gridShift 20s linear infinite" } as React.CSSProperties,
  glow: { position: "absolute", width: 560, height: 560, borderRadius: "50%",
    background: "radial-gradient(circle, rgba(106,247,209,0.25), rgba(0,0,0,0))",
    filter: "blur(60px)", pointerEvents: "none" },
  nav: { maxWidth: 1120, margin: "0 auto", padding: "22px 20px",
  ...
  heroWrap: { maxWidth: 1120, margin: "28px auto 0", padding: "0 20px",
    display: "grid", gridTemplateColumns: "1.2fr .8fr", gap: 24 },
  ...
  ctaPrimary: { display: "inline-block", padding: "10px 14px", borderRadius: 12, textDecoration: "none", fontWeight: 800,
    background: "#00ffc2", color: "#001018", boxShadow: "0 0 18px rgba(106,247,209,.35)" },
  ctaGhost: { display: "inline-block", padding: "10px 14px", borderRadius: 12, textDecoration: "none", fontWeight: 700,
  ...
  card: { padding: 16, borderRadius: 16, background: "rgba(16,19,20,.88)", border: "1px solid rgba(255,255,255,.18)",
    boxShadow: "0 0 18px rgba(0,0,0,.25)", display: "grid", gap: 10 },
  cardIcon: { width: 44, height: 44, borderRadius: 12, border: "2px solid rgba(106,247,209,.4)",
    display: "grid", placeItems: "center", background: "rgba(0,0,0,.25)" },
  cardDot: { width: 10, height: 10, borderRadius: 999 },
  footer: { maxWidth: 1120, margin: "40px auto 30px", padding: "0 20px",
    display: "flex", justifyContent: "space-between", opacity: .7, fontSize: 13 },
};
