...
export default function Improve() {
 ...
 /* Save → PATCH chatbots/[id] */ async function saveEdits() { if (!userId || !selectedId) return; setSaving(true); try { const prev = list.find(b => b.id === selectedId) || { system, model, temperature }; // snapshot first const v: Version = { id: v_${Date.now()}, ts: Date.now(), label: snapshotLabel( { system: prev.system, model: prev.model, temperature: prev.temperature }, { system, model, temperature } ), name, model, temperature, system }; const next = [v, ...versions].slice(0, 80); setVersions(next); try { localStorage.setItem(versionsKey(userId, selectedId), JSON.stringify(next)); } catch {} // persist await fetch(/api/chatbots/${selectedId}?ownerId=${encodeURIComponent(userId)}, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'x-owner-id': userId }, credentials: 'include', body: JSON.stringify({ name, model, temperature, system }), }); setList(cur => cur.map(b => b.id === selectedId ? { ...b, name, model, temperature, system, updatedAt: new Date().toISOString() } : b )); setDirty(false); } catch (e) { alert('Failed to save'); } finally { setSaving(false); } }
