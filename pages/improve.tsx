...
// - "Add Rule" moved up next to the toggle; Pre/Post prompt moved inside the panel.
// - Attachments are now real: images are sent to the model via image_url parts.
...
/* Snapshot label (kept simple to avoid “undefined” issues) */
function snapshotLabel(
  prev: { system: string; model: string; temperature: number },
  next: { system: string; model: string; temperature: number }
): string {
  const deltas: string[] = [];
  if (prev.model !== next.model) deltas.push(`model→${next.model}`);
  if (Math.abs(prev.temperature - next.temperature) > 1e-9) {
    deltas.push(`temp→${next.temperature.toFixed(2)}`);
  }
  if ((prev.system || '').trim() !== (next.system || '').trim()) {
    // quick heuristics for nicer names
    if (/only\s+yes\/?no|strict\s+yes|strict\s+no/i.test(next.system)) deltas.push('yes/no only');
    else if (/concise|short|brief/i.test(next.system)) deltas.push('more concise');
    else if (/json|schema|valid/i.test(next.system)) deltas.push('json bias');
    else if (/cite|source/i.test(next.system)) deltas.push('cites sources');
    else deltas.push('prompt edited');
  }
  return deltas.length ? deltas.join(' · ') : 'minor edit';
}
...
export default function Improve() {
  ...
  async function saveEdits() {
    ...
    try {
      ...
      const v: Version = {
        ...
        label: snapshotLabel(
          { system: (prev as any).system, model: (prev as any).model, temperature: (prev as any).temperature },
  ...
  async function onSelectFiles(filesList: FileList | null) {
    if (!filesList || !filesList.length) return;
    const arr = Array.from(filesList).slice(0, 16);
    const atts = await Promise.all(arr.map(fileToAttachment));
    for (const a of atts) {
      if (a.mime.startsWith('image/')) setPics(prev => [...prev, a].slice(0, 16));
      else if (a.mime.startsWith('video/')) setVids(prev => [...prev, a].slice(0, 8));
      else setFiles(prev => [...prev, a].slice(0, 16));
    }
    setTrayOpen(false);
  }

  /* Build OpenAI vision-friendly "content" for the new user message */
  function buildUserContent(text: string, atts: Attachment[]) {
    const parts: any[] = [];
    const trimmed = (text || '').trim();
    if (trimmed) parts.push({ type: 'text', text: trimmed });

    // images → image_url parts; videos/files → brief text note (Chat Completions doesn't ingest video)
    for (const a of atts) {
      if (a.mime.startsWith('image/') && a.url) {
        parts.push({ type: 'image_url', image_url: { url: a.url } });
      } else {
        parts.push({ type: 'text', text: `[attached ${a.mime || 'file'}: ${a.name}]` });
      }
    }

    // If there are no attachments and only text, return a plain string for compactness.
    if (parts.length === 1 && parts[0].type === 'text' && !atts.length) return trimmed;
    return parts;
  }
  ...
  const sendLane = useCallback(async (which: 'A' | 'B', text: string, atts: Attachment[]) => {
    ...
    const historyForDisplay: ChatMsg[] = [...base];
    const historyForUpstream: any[] = base.map(m => ({ role: m.role, content: m.content }));

    const userContent = buildUserContent(text, atts);
    historyForUpstream.push({ role: 'user', content: userContent });

    // optimistic user bubble (keep it readable)
    const chip =
      atts.length > 0
        ? ` (sent ${atts.length} attachment${atts.length > 1 ? 's' : ''})`
        : '';
    const echo = (text || '').trim() || (atts.length ? chip.trim() : '');
    if (echo) {
      if (which === 'A') setMsgsA(cur => [...cur, { role: 'user', content: (text || '').trim() || chip.trim() }]);
      else setMsgsB(cur => [...cur, { role: 'user', content: (text || '').trim() || chip.trim() }]);
    }
...
if (resp.ok && typeof data?.content === 'string') {
  ...
  reply = String(err);
      ...
      if (which === 'A') {
        ...
        updateLocalMemory([...historyForDisplay, { role: 'user', content: echo }, { role: 'assistant', content: reply }]);
      } else {
    ...
    >
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-5" style={{ height: 'calc(100vh - 62px)' }}>
        <div className="grid gap-3" style={{ gridTemplateColumns: '300px 1fr 300px', height: '100%' }}>
          ...
          <section className="h-full grid gap-3" style={{ gridTemplateRows: 'auto 1fr auto' }}>
            {/* Editor header row */}
            <div className="p-3" style={PANEL}>
              ...
              {showPromptPanel && (
                <div className="mt-3">
                  <div className="p-3 rounded-md border" style={{ ...CARD }}>
                    <VapiScramble key={scrambleKey} text={`You’re called ${name || 'test'}. ${previewSentence}`} />
                  </div>
            ...
            <div className="p-3 grid gap-3" style={{ ...PANEL, gridTemplateRows: 'auto 1fr auto' }}>
              ...
              <div className="relative">
                ...
                {/* Current attachments quick preview */}
            ...
            <aside className="p-3" style={PANEL}>
              <div className="text-sm opacity-70 mb-2">Versions</div>
              {versions.length === 0 ? (
                  ...
                  {versions.map(v => (
                    <li key={v.id}>
                      <div
                        ...
                        className="rounded-md border p-2 text-sm cursor-grab"
                        style={CARD}
                      >
                        ...
                        <div className="mt-2 flex gap-2">
                          <button
                            ...
                            style={CARD}
                            onClick={() => { setLaneB(v); setMsgsB([]); setActiveLane('A'); }}
                          >Load</button>
                          <button
                            className="px-2 py-1 rounded text-xs"
                            style={CARD}
