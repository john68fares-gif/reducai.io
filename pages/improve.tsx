...
export default function Improve() {
  ...
  // prompt editor (single field to keep your current UX; rules appended via helper)
  ...
  const rulesText = quickRules.length ? '\n' + quickRules.map(r => `- ${r}`).join('\n') : '';
  ...
  async function saveEdits() {
    ...
    try {
      ...
      const effectiveSystem = [system, rulesText].filter(Boolean).join('\n').trim();
      ...
      setLastSavedInfo(`Saved @ ${fmt(Date.now())}`);
    } catch {
  ...
  function saveSnapshotNearPrompt() {
    ...
    setVersions(nextV);
    try { localStorage.setItem(versionsKey(userId, selected.id), JSON.stringify(nextV)); } catch {}
    setLastSavedInfo(`Snapshot @ ${fmt(v.ts)}`);
  }
  ...
  function addQuickRule(line: string) {
    ...
    setQuickRules(prev => {
      const next = [...prev, val];
      const effective = [system, next.map(r => `- ${r}`).join('\n')].filter(Boolean).join('\n');
      if (activeLane) {
  ...
  /* ===== Chat (Support contract, per-lane) ===== */
  ...
  return (
    <div className="h-screen w-full overflow-hidden text-slate-100">
      {/* Top bar */}
      <header className="sticky top-0 z-30 px-4 py-3 border-b border-white/10 bg-black/40 backdrop-blur" style={{ height: 56 }}>
        <div className="max-w-[1600px] mx-auto h-full flex items-center gap-3">
          ...
          <span className="ml-2 text-[11px] px-2 py-[2px] rounded-full border" style={{ borderColor: 'rgba(255,255,255,0.10)', color: dirty ? ACCENT : '#94a3b8' }}>
            {dirty ? 'Unsaved changes' : 'Saved'}
          </span>
          <div className="ml-auto flex items-center gap-2">
            <SmallButton onClick={saveEdits} disabled={!dirty} style={{ borderColor: dirty ? ACCENT : 'rgba(255,255,255,0.10)' }}>Save Agent</SmallButton>
      ...
      <div className="max-w-[1600px] mx-auto px-4" style={{ height: 'calc(100vh - 56px)' }}>
        <div className="grid gap-3" style={{ gridTemplateColumns: '300px 1fr', height: '100%' }}>
          ...
          <aside className="h-full flex flex-col" style={{ background: 'rgba(13,15,17,0.92)', border: '1px solid rgba(106,247,209,0.18)', borderRadius: 12 }}>
            <div className="p-3 border-b border-white/10 flex items-center gap-2">
              <div className="font-medium">Versions</div>
              <input value={versionFilter} onChange={(e) => setVersionFilter(e.target.value)} placeholder="Search…" className="ml-auto px-2 py-1 text-xs rounded-md bg-white/5 border border-white/10 outline-none" />
            </div>
            <div className="flex-1 overflow-auto p-2">
              {(!versions || versions.length === 0) ? (
                <div className="text-xs opacity-70 p-3">No snapshots yet. Use <b>Save snapshot</b> below to create one.</div>
              ) : (
                <ul className="space-y-2">
                  {versions
                    .filter(v => !versionFilter || v.label.toLowerCase().includes(versionFilter.toLowerCase()))
                    .map(v => (
                      <li key={v.id} className="p-2 rounded-md" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <SmallButton onClick={() => { setName(v.name); setModel(v.model); setTemperature(v.temperature); setSystem(clean(v.system)); setDirty(true); }}>Restore</SmallButton>
            ...
            <div className="border-t border-white/10 p-2">
              ...
              <div className="space-y-1 max-h-40 overflow-auto">
                {loading ? (
                  ...
                  list.map(b => (
                    <button key={b.id} onClick={() => setSelectedId(b.id)} className="w-full text-left text-xs px-2 py-1 rounded-md border"
                      style={{ borderColor: selectedId === b.id ? ACCENT : 'rgba(255,255,255,0.10)', background: selectedId === b.id ? 'rgba(0,255,194,0.08)' : 'transparent' }}>
          ...
          <section className="h-full grid" style={{ gridTemplateRows: 'auto 1fr' }}>
            {/* compact editor */}
            <div className="p-3 flex items-center gap-3 border border-white/10 rounded-md" style={{ background: 'rgba(13,15,17,0.92)', borderColor: 'rgba(106,247,209,0.18)' }}>
              <div className="grid gap-2" style={{ gridTemplateColumns: '1.2fr 0.9fr 0.8fr' }}>
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Agent name" className="px-2 py-2 rounded-md bg-white/5 border border-white/10 outline-none" />
              ...
              <div className="ml-auto flex items-center gap-2 text-xs">
                <div className="px-2 py-1 rounded-md border border-white/10">est {estimateTokens([system, rulesText].join('\n'))} tok</div>
                <SmallButton onClick={() => setShowPrompt(s => !s)}>{showPrompt ? 'Hide Prompt' : 'Show Prompt'}</SmallButton>
              </div>
            </div>

            {/* body */}
            <div className="grid gap-3 mt-3" style={{ gridTemplateColumns: showPrompt ? 'minmax(260px, 380px) 1fr' : '1fr', height: 'calc(100% - 0px)' }}>
              {/* Prompt panel */}
              {showPrompt && (
                <div className="h-full flex flex-col" style={{ background: 'rgba(13,15,17,0.92)', border: '1px solid rgba(106,247,209,0.18)', borderRadius: 12 }}>
                  ...
                  <div className="p-3 space-y-3 overflow-auto">
                    ...
                    <textarea value={system} onChange={(e) => {
                      const val = e.target.value; setSystem(val);
                      if (activeLane) {
                        const eff = [val, rulesText].filter(Boolean).join('\n');
                    ...
                    <div className="text-xs opacity-70">Quick rules (bullets)</div>
                    <div className="flex items-center gap-2">
                      <input id="quickRule" placeholder="e.g., Cite sources with links" className="flex-1 px-2 py-2 rounded-md bg-white/5 border border-white/10 outline-none text-xs" />
                      <SmallButton onClick={() => { const el = document.getElementById('quickRule') as HTMLInputElement | null; if (!el) return; addQuickRule(el.value); el.value = ''; }}>Add Rule</SmallButton>
                    </div>
                    <ul className="space-y-1">
                      {quickRules.map((r, i) => (
                        <li key={i} className="text-xs flex items-center justify-between px-2 py-1 rounded-md bg-white/5 border border-white/10">
                          <span className="opacity-90">{r}</span>
                          <button onClick={() => removeQuickRule(i)} className="text-[11px] opacity-70 hover:opacity-100">remove</button>
                        </li>
                      ))}
                      {quickRules.length === 0 && <li className="text-[11px] opacity-60">No quick rules yet.</li>}
                    </ul>

                    <div className="flex items-center gap-2 pt-2 border-t border-white/10">
                      <button onClick={saveSnapshotNearPrompt} className="px-2 py-1 rounded-md border" style={{ borderColor: ACCENT, color: ACCENT }}>Save snapshot</button>
                      <label className="text-xs opacity-80 flex items-center gap-2">
                        <input type="checkbox" checked={autoSnapshot} onChange={(e) => setAutoSnapshot(e.target.checked)} /> Auto snapshot on rule add
                      </label>
                      {lastSavedInfo && <div className="text-[11px] opacity-60 ml-2">{lastSavedInfo}</div>}
                    </div>

                    <div className="pt-2 border-t border-white/10">
                      <div className="text-xs opacity-70">Notes (optional)</div>
                      <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full h-20 px-2 py-2 rounded-md bg-white/5 border border-white/10 outline-none text-xs" />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="h-full flex flex-col" style={{ background: 'rgba(13,15,17,0.92)', border: '1px solid rgba(106,247,209,0.18)', borderRadius: 12 }}>
                <div className="p-3 border-b border-white/10 flex items-center gap-2">
                  ...
                  <div className="text-xs opacity-70">Compare lanes. Drag/open versions to test different snapshots.</div>
                  <div className="ml-auto flex items-center gap-2">
                    <SmallButton onClick={() => {
                      const sys = [system, rulesText].filter(Boolean).join('\n');
                      const v: Version = { id: `v_live_${Date.now()}`, ts: Date.now(), label: summarizeTitle(sys), name, model, temperature, system: sys };
                ...
                <div className="flex-1 overflow-hidden p-2">
                  <div className="grid gap-2 h-full" style={{ gridTemplateColumns: `repeat(${Math.max(1, lanes.length)}, minmax(280px, 1fr))` }}>
                    {lanes.map(l => (
                      <div key={l.id} className="h-full flex flex-col rounded-md border" style={{ borderColor: activeLaneId === l.id ? ACCENT : 'rgba(255,255,255,0.10)', background: 'rgba(255,255,255,0.02)' }}>
                        <div className="px-3 py-2 flex items-center gap-2 border-b border-white/10">
                          <button onClick={() => setActiveLaneId(l.id)} className="text-left flex-1">
                            <div className="text-sm font-medium truncate" style={{ color: activeLaneId === l.id ? ACCENT : '#e2e8f0' }}>{l.title || 'Variant'}</div>
                        ...
                        {!!(l.attachments && l.attachments.length) && (
                          <div className="px-3 pt-2 flex gap-2 overflow-x-auto">
                            {l.attachments!.map(att => (
                              <div key={att.id} className="min-w-[120px] max-w-[180px] rounded-md border border-white/10 bg-white/5 p-2 text-[11px]">
                                {att.url && att.mime.startsWith('image/') && <img src={att.url} alt={att.name} className="w-full h-24 object-cover rounded" />}
                                {att.url && att.mime.startsWith('video/') && <video className="w-full h-24 rounded" src={att.url} controls muted />}
                                {!att.url && <div className="h-24 grid place-items-center opacity-60">file</div>}
                        ...
                        <div className="flex-1 overflow-auto px-3 py-2 space-y-2">
                          {l.msgs.length === 0 ? (
                            ...
                            <div key={i} className={`text-sm whitespace-pre-wrap ${m.role === 'user' ? '' : 'opacity-85'}`}>
                              <b style={{ color: m.role === 'user' ? '#e2e8f0' : ACCENT }}>{m.role === 'user' ? 'You' : 'AI'}:</b>{' '}
                ...
                {/* input */}
                <ChatInput onSend={(t, mode, files) => sendMessage(t, mode, files)} canSendBoth={lanes.length > 1} />
              </div>
            </div>
          </section>
        </div>
...
/* ---------------- ChatInput (attachments preview only; add real upload when ready) ---------------- */
function ChatInput({
  onSend,
  canSendBoth
}: {
  ...
  React.useEffect(() => {
    ...
    const onDrop = (e: DragEvent) => { e.preventDefault(); setDragOver(false); addFiles(e.dataTransfer?.files || null); };
    ...
    <div ref={dropRef} className={`border-t border-white/10 p-2 ${dragOver ? 'bg-white/5' : ''}`}>
      {attachments.length > 0 && (
        <div className="flex items-center gap-2 pb-2 overflow-x-auto">
          {attachments.map(att => (
            <div key={att.id} className="min-w-[120px] max-w-[180px] flex items-center gap-2 px-2 py-1 rounded-md border bg-white/5" style={{ borderColor: 'rgba(255,255,255,0.10)' }}>
              {att.url && att.mime.startsWith('image/') && <img src={att.url} alt={att.name} className="w-10 h-10 object-cover rounded" />}
              {att.url && att.mime.startsWith('video/') && <video className="w-10 h-10 rounded" src={att.url} muted />}
              {!att.url && <div className="w-10 h-10 grid place-items-center text-[10px] opacity-60">file</div>}
              <span className="text-[11px] truncate max-w-[120px]">{att.name}</span>
              <button className="text-[11px] opacity-70 hover:opacity-100" onClick={() => setAttachments(prev => prev.filter(a => a.id !== att.id))}>✕</button>
            </div>
          ))}
        </div>
      )}
      <div className="flex items-end gap-2">
        ...
        <button onClick={() => send('active')} className="px-3 py-2 text-xs rounded-md border" style={{ borderColor: ACCENT, color: ACCENT }}>Send</button>
      </div>
