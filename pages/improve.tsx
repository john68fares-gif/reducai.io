// apps/web/app/improve.tsx — Improve with Support Chat Functions (FULL FILE)
// WHAT'S CHANGED VS YOUR EXISTING IMPROVE:
// - Chat pipeline now uses the SAME route/contract as Support: POST /api/support/ask -> { ok, message }
// - Pulled over Support's sanitize(), TypingDots(), and simple send() semantics (no "can't comply" spam)
// - Live prompt updates (rule line + textarea) immediately affect active lane (same as before)
// - Only chat list scrolls; page is fixed-height
// - Tiny hint text; horizontal attachments
// - No hardcoded page background; respects your site theme
//
// Drop this in place of apps/web/app/improve.tsx if you want Improve to behave like Support during tests.
// If you want to keep your old file, save this as improve.with-support-chat.tsx and route to it for A/B.
...
/* ========================= Theme bits ========================= */
const ACCENT = '#00ffc2'; // site accent; minimal use
...
/* === Support.tsx chat helpers brought over === */
...
/* Short human-friendly title derived from system */
...
/* Auto name for version snapshots */
...
export default function Improve() {
    ...
    try {
      ...
      if (rawS) {
        ...
        const l: Lane = {
          ...
          msgs: [{ role: 'assistant', text: 'Hi, provide a detailed message to test this variant.' }],
          sending: false,
        };
        setLanes([l]);
  ...
  async function saveEdits() {
    ...
    try {
      ...
      const candidate: BotRow = { ...prev, name, model, temperature, system: clean(system) };
      const v: Version = {
        id: `v_${Date.now()}`,
        ts: Date.now(),
        label: autoLabel(prev, candidate),
        name, model, temperature, system: clean(system),
      };
      ...
      try {
        await fetch(`/api/chatbots/${selected.id}?ownerId=${encodeURIComponent(userId)}`, {
          ...
          body: JSON.stringify({ name, model, temperature, system: clean(system) }),
        });
      } catch {}
      setDirty(false);
      // reflect in first lane
      setLanes(ls => ls.map((l, i) =>
        i === 0 ? { ...l, system: clean(system), model, temperature, title: summarizeTitle(system) } : l));
    } catch {
  ...
  /* Apply a new rule line to the system prompt (live for active lane) */
  function applyRuleLine(line: string) {
    const add = clean(line);
    if (!add) return;
    const next = `${clean(system)}\\n- ${add}`.trim();
    setSystem(next);
    setDirty(true);
    if (activeLane) {
  ...
  /* ===== Chat: SAME contract as Support (no streaming) ===== */
            ...
            <div className="grid gap-3 mt-3" style={{ gridTemplateColumns: showPrompt ? 'minmax(260px, 360px) 1fr' : '1fr', height: 'calc(100% - 0px)' }}>
              ...
              <div className="h-full flex flex-col" style={{ background: 'rgba(13,15,17,0.92)', border: '1px solid rgba(106,247,209,0.18)', borderRadius: 12 }}>
                <div className="p-3 border-b border-white/10 flex items-center gap-2">
                  ...
                  <div className="text-xs opacity-70">Compare lanes. Click a lane to target it. “Send to Both” compares outputs.</div>
                  <div className="ml-auto flex items-center gap-2">
                    <SmallButton
                      onClick={() => {
                        const v: Version = {
                  ...
                  <div className="grid gap-2 h-full" style={{ gridTemplateColumns: `repeat(${Math.max(1, lanes.length)}, minmax(280px, 1fr))` }}>
                    {lanes.map(l => (
                      ...
                      >
                        ...
                        <div className="flex-1 overflow-auto px-3 py-2 space-y-2 custom-scroll">
                          {l.msgs.map((m, i) => (
                          ...
                          {l.msgs.length === 0 && <div className="text-xs opacity-60">No messages yet.</div>}
                        </div>
                      </div>
                    ))}
                  </div>
